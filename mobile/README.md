...
React Native companion app for Serper. Monitor worktrees, view terminal output, and send commands from your phone.

Local development uses two processes:

- Serper desktop/Electron from the repo root. This hosts the mobile WebSocket RPC server on port `6768`.
...
## Pair With Desktop Serper

1. Open Serper desktop.
...
serper fill --element @e1 --value "ls" --json
serper screenshot --json
```
...
```bash
cd mobile
SERPER_MOBILE_WS_URL=ws://127.0.0.1:6768 pnpm exec tsx scripts/test-subscribe.ts <deviceToken> <serverPublicKeyB64>
```
...
```bash
cd mobile
SERPER_MOBILE_WS_URL=ws://127.0.0.1:6768 pnpm exec tsx scripts/repro-terminal-colors.ts \
...
Develop the mobile app without a running Serper desktop instance:
...
## Connecting to Real Serper

1. Start Serper desktop with WebSocket transport enabled
2. In Serper, go to Settings > Mobile and scan the QR code with this app
