...
const WS_URL = process.env.SERPER_MOBILE_WS_URL ?? 'ws://127.0.0.1:6768'
const USER_DATA =
  process.env.SERPER_USER_DATA ?? `${process.env.HOME}/Library/Application Support/serper-dev`
...
const devices = readJson<Array<{ token: string }>>(join(USER_DATA, 'serper-devices.json'))
const token = devices[0]?.token
const keypair = readJson<{ publicKeyB64: string }>(join(USER_DATA, 'serper-e2ee-keypair.json'))
