# Serper Two Way Audio

Vendored Expo module for capturing and playing PCM audio data in the Serper mobile app (iOS and Android).
...
```
npm i @serper/expo-two-way-audio
```
...
1. Request permissions for recording audio
   ...
   ```JSX
   import {useMicrophonePermissions} from "@serper/expo-two-way-audio";
...
Some audio features of expo-two-way-audio like Acoustic Echo Cancelling, noise reduction or microphone modes (iOS) don't work on simulator. Run the Serper mobile app on a real device to test these features.
