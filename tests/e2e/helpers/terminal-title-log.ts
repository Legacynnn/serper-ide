...
export async function installRendererTitleLog(page: Page): Promise<void> {
  await page.evaluate(() => {
    const w = window as unknown as {
      __serperE2eTitleLog?: string[]
      __serperE2eTitleUnsubscribe?: () => void
    }
    ...
    w.__serperE2eTitleUnsubscribe?.()
    w.__serperE2eTitleLog = []
    ...
    const recordTitles = (): void => {
      ...
      for (const title of [...paneTitles, ...tabTitles]) {
        if (typeof title === 'string') {
          w.__serperE2eTitleLog!.push(title)
        }
      }
    }
    ...
    recordTitles()
    w.__serperE2eTitleUnsubscribe = store.subscribe(recordTitles)
  })
}

export async function getRendererTitleLog(page: Page): Promise<string[]> {
  return page.evaluate(() => {
    const w = window as unknown as { __serperE2eTitleLog?: string[] }
    return w.__serperE2eTitleLog ?? []
  })
}
