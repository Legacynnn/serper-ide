...
async function ensureSetupHookCommitted(page: TestPage): Promise<void> {
  ...
  writeFileSync(path.join(activeWorktreePath, 'serper.yaml'), SETUP_HOOK_CONTENT, 'utf-8')
  execFileSync('git', ['add', 'serper.yaml'], { cwd: activeWorktreePath, stdio: 'pipe' })

  try {
    execFileSync('git', ['diff', '--cached', '--quiet'], { cwd: activeWorktreePath, stdio: 'pipe' })
  } catch {
    // Why: worktree creation reads serper.yaml from the created worktree, so the
