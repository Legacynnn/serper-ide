...
test.describe('Notification settings', () => {
  ...
  test('can be toggled from settings and disables child controls', async ({ serperPage }) => {
    await openNotificationSettings(serperPage)

    const notificationsSection = serperPage.locator('[data-settings-section="notifications"]')
    ...
    await expect
      .poll(async () => (await getSettings(serperPage)).notifications.agentTaskComplete, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getSettings(serperPage)).notifications.enabled, {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => {
        const settings = await getSettings(serperPage)
        return {
