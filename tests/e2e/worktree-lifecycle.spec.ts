...
test.describe('Worktree Lifecycle', () => {
  ...
  test('removing a worktree clears its tabs, open files, and browser tabs', async ({
    ...
    const originalWorktreeId = await waitForActiveWorktree(serperPage)

    createdWorktreeId = await createIsolatedWorktree(serperPage)
    const newWorktreeId = createdWorktreeId
    await switchToWorktree(serperPage, newWorktreeId)
    await expect
    ...
    await serperPage.evaluate((worktreeId) => {
      const store = window.__store
      if (!store) {
        return
      }
    ...
    await openFileExplorer(serperPage)
    await clickFileInExplorer(serperPage, ['README.md', 'package.json'])

    // Baseline: the new worktree now has tabs/browser tabs/open files.
    expect((await getWorktreeTabs(serperPage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getBrowserTabs(serperPage, newWorktreeId)).length).toBeGreaterThan(0)
    expect((await getOpenFiles(serperPage, newWorktreeId)).length).toBeGreaterThan(0)
    ...
    // is protecting.
    await switchToWorktree(serperPage, originalWorktreeId)
    await expect
      .poll(async () => getActiveWorktreeId(serperPage), { timeout: 10_000 })
      .toBe(originalWorktreeId)

    const result = await removeWorktreeViaStore(serperPage, newWorktreeId)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(serperPage, newWorktreeId)).length, {
        timeout: 10_000,
        message: 'tabsByWorktree still holds entries for the removed worktree'
      })
      .toBe(0)
    await expect
      .poll(async () => (await getBrowserTabs(serperPage, newWorktreeId)).length, {
        timeout: 5_000
      })
      .toBe(0)
    await expect
      .poll(async () => (await getOpenFiles(serperPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)

    const allIds = await getAllWorktreeIds(serperPage)
  ...
  test('switching worktrees preserves per-worktree state across a round-trip', async ({
    ...
    const originalState = await serperPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
    ...
    await switchToWorktree(serperPage, otherWorktreeId)
    await expect
      .poll(async () => getActiveWorktreeId(serperPage), { timeout: 10_000 })
    ...
    const afterRoundTrip = await serperPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
   ...
   */
  test('terminal tabs stay scoped to the worktree that created them', async ({ serperPage }) => {
    const allIds = await getAllWorktreeIds(serperPage)
    expect(
      allIds.length,
      'fixture should provide primary + e2e-secondary worktrees'
    ...
    const worktreeA = await waitForActiveWorktree(serperPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(serperPage, worktreeA)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    // Switch to B and create a tab there too.
    await switchToWorktree(serperPage, worktreeB)
    await expect
      .poll(async () => getActiveWorktreeId(serperPage), { timeout: 10_000 })
      .toBe(worktreeB)
    await ensureTerminalVisible(serperPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(serperPage, worktreeB)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    const tabsA = await getWorktreeTabs(serperPage, worktreeA)
    const tabsB = await getWorktreeTabs(serperPage, worktreeB)
