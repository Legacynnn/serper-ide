 ...
 * - PR #677 (`return to Serper landing screen after closing last terminal`)
...
test.describe('Tab Close Navigation', () => {
   ...
   */
  test('closing the active editor tab activates its visual neighbor', async ({ serperPage }) => {
    const worktreeId = await waitForActiveWorktree(serperPage)

    const fileIds = await openSeededEditorTabs(serperPage, worktreeId, [
      'package.json',
      'README.md',
      'tsconfig.json'
    ])
    ...
    await setActiveFile(serperPage, fileIds[1])
    await expect.poll(async () => getActiveFileId(serperPage), { timeout: 3_000 }).toBe(fileIds[1])

    await closeFile(serperPage, fileIds[1])
    ...
    await expect
      .poll(async () => getActiveFileId(serperPage), {
        timeout: 5_000,
   ...
   */
  test('closing the active diff tab activates a still-open neighbor', async ({ serperPage }) => {
    const worktreeId = await waitForActiveWorktree(serperPage)

    // Seed two editor tabs + one diff tab in the same worktree.
    const editorIds = await openSeededEditorTabs(serperPage, worktreeId, [
      'package.json',
      'README.md'
    ])
    expect(editorIds.length).toBe(2)

    const diffId = await serperPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
        return null
      }
    ...
    await expect.poll(async () => getActiveFileId(serperPage), { timeout: 3_000 }).toBe(diffId)

    await closeFile(serperPage, diffId!)
   ...
   */
  test('closing the last visible surface returns the app to Landing', async ({ serperPage }) => {
    const worktreeId = await waitForActiveWorktree(serperPage)
    ...
    const editorIds = await openSeededEditorTabs(serperPage, worktreeId, ['package.json'])
    expect(editorIds.length).toBe(1)

    await setActiveFile(serperPage, editorIds[0])
    await expect
      .poll(async () => getActiveFileId(serperPage), { timeout: 3_000 })
      .toBe(editorIds[0])
    ...
    const surfaceCounts = await serperPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
    ...
    await closeFile(serperPage, editorIds[0])
