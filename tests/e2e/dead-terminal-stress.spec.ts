...
test.describe('Dead Terminal Stress @headful', () => {
   ...
   */
  test('@headful setup-split with forced WebGL context loss recovers', async ({ serperPage }) => {
    ...
    for (let i = 0; i < STRESS_ITERATIONS; i++) {
      const newId = await createAndActivateWorktreeWithSetup(serperPage, `ctxloss-${i}`, 'vertical')
      ...
      const lostCount = await serperPage.evaluate(() => {
      ...
      await waitForAllPanesToHaveContent(serperPage, `ctxloss-${i} after context loss`)
   ...
   */
  test('@headful rapid worktree switching during setup-split lifecycle', async ({ serperPage }) => {
    test.setTimeout(120_000)
    const homeWorktreeId = await waitForActiveWorktree(serperPage)
    await waitForActiveTerminalManager(serperPage, 30_000)

    for (let i = 0; i < 3; i++) {
      const newId = await createAndActivateWorktreeWithSetup(serperPage, `rapid-${i}`, 'vertical')
      createdWorktreeIds.push(newId)
      ...
      await waitForActiveTerminalManager(serperPage, 30_000)
      await waitForPaneCount(serperPage, 2, 15_000)
      await waitForAllPanesToHaveContent(serperPage, `rapid-${i} after return`)
