...
test.describe('Workspace Back/Forward Navigation', () => {
  ...
  test('buttons are hidden outside the terminal view', async ({ serperPage }) => {
    await expect(await getBackButton(serperPage)).toBeVisible()
    await expect(await getForwardButton(serperPage)).toBeVisible()
    ...
    await expect(await getBackButton(serperPage)).toHaveCount(0)
    await expect(await getForwardButton(serperPage)).toHaveCount(0)
  ...
  test('both buttons disabled at cold start with a single history entry', async ({
    ...
    const activeId = await getActiveWorktreeId(serperPage)
    expect(activeId).not.toBeNull()

    await resetNavHistory(serperPage)
    await seedVisit(serperPage, activeId!)

    const back = await getBackButton(serperPage)
    const forward = await getForwardButton(serperPage)
    await expect(back).toBeDisabled()
    await expect(forward).toBeDisabled()
  })

  test('clicking Back and Forward walks the history stack', async ({ serperPage }) => {
    const worktreeIds = await getAllWorktreeIds(serperPage)
    ...
    await resetNavHistory(serperPage)
    await seedVisit(serperPage, primaryId)
    await seedVisit(serperPage, secondaryId)
    ...
    const primaryRow = serperPage.locator(
      `[id="worktree-list-option-${encodeURIComponent(primaryId)}"]`
    )
    const secondaryRow = serperPage.locator(
      `[id="worktree-list-option-${encodeURIComponent(secondaryId)}"]`
    )

    await back.click()
    await expect
      .poll(async () => getActiveWorktreeId(serperPage), {
  ...
  test('re-activating the current worktree is a no-op (dedupe)', async ({ serperPage }) => {
    const activeId = await getActiveWorktreeId(serperPage)
    expect(activeId).not.toBeNull()
    ...
    const snapshot = await getNavHistorySnapshot(serperPage)
    expect(snapshot.history).toEqual([activeId])
    expect(snapshot.index).toBe(0)
    await expect(await getBackButton(serperPage)).toBeDisabled()
  })

  test('new navigation after going back truncates the forward stack', async ({ serperPage }) => {
    const worktreeIds = await getAllWorktreeIds(serperPage)
    ...
    await seedVisit(serperPage, primaryId)
    await seedVisit(serperPage, secondaryId)
    await (await getBackButton(serperPage)).click()
    await expect.poll(() => getActiveWorktreeId(serperPage)).toBe(primaryId)

    // Forward button is live — a forward entry exists.
    await expect(await getForwardButton(serperPage)).toBeEnabled()
    ...
    await expect(await getForwardButton(serperPage)).toBeDisabled()
  })
  ...
  test(`${isMac ? 'Cmd' : 'Ctrl'}+Alt+Left/Right shortcuts walk history`, async ({
    ...
    await serperPage.keyboard.press(`${mod}+Alt+ArrowLeft`)
    await expect
      .poll(async () => getActiveWorktreeId(serperPage), {
    ...
    await serperPage.keyboard.press(`${mod}+Alt+ArrowRight`)
    await expect
      .poll(async () => getActiveWorktreeId(serperPage), {
  ...
  test('shortcut is a no-op in settings view', async ({ serperPage }) => {
    const worktreeIds = await getAllWorktreeIds(serperPage)
    ...
    await expect
      .poll(async () => serperPage.evaluate(() => window.__store!.getState().activeView))
      .toBe('settings')

    const idBefore = await getActiveWorktreeId(serperPage)
    await serperPage.evaluate(() => document.body.focus())
    await serperPage.keyboard.press(`${mod}+Alt+ArrowLeft`)

    // Give any erroneous nav a beat to land, then assert the active worktree
    // and the slice index both stayed put.
    await serperPage.waitForTimeout(150)
    expect(await getActiveWorktreeId(serperPage)).toBe(idBefore)
    const snapshot = await getNavHistorySnapshot(serperPage)
    expect(snapshot.index).toBe(1)
  })
})
