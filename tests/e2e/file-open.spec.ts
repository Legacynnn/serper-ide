...
test.describe('File Open & Markdown Preview', () => {
   ...
   * - you can open files (from the right sidebar)
   */
  test('opening the right sidebar shows file explorer', async ({ serperPage }) => {
    await openFileExplorer(serperPage)

    // Why: the load-bearing check is that `FileExplorer` actually mounted.
    // `data-serper-explorer-shell` is the stable marker the component renders
    ...
    // painted empty.
    await expect(serperPage.locator('[data-serper-explorer-shell]')).toBeVisible({ timeout: 5_000 })
  })
   ...
   * - you can open files (from the right sidebar)
   */
  test('clicking a file in the file explorer opens it in an editor tab', async ({ serperPage }) => {
    const worktreeId = (await getActiveWorktreeId(serperPage))!
    await openFileExplorer(serperPage)

    const filesBefore = await getOpenFiles(serperPage, worktreeId)

    // Click a known non-directory file
    const clickedFile = await clickFileInExplorer(serperPage, [
      'package.json',
      'tsconfig.json',
      '.gitignore',
      'README.md'
    ...
    await expect.poll(async () => getActiveTabType(serperPage), { timeout: 5_000 }).toBe('editor')

    // There should be a new open file
    await expect
      .poll(async () => (await getOpenFiles(serperPage, worktreeId)).length, { timeout: 5_000 })
    ...
    await expect(serperPage.locator('.editor-header-path').first()).toContainText(clickedFile!, {
      timeout: 20_000
    })
  })
   ...
   * - you can open .md files and they show up as preview (from the right sidebar)
   */
  test('opening a .md file shows markdown content', async ({ serperPage }) => {
    await openFileExplorer(serperPage)
    const clickedFile = await clickFileInExplorer(serperPage, ['README.md', 'CLAUDE.md'])
    expect(clickedFile).not.toBeNull()

    // Wait for the editor tab to become active
    await expect.poll(async () => getActiveTabType(serperPage), { timeout: 5_000 }).toBe('editor')

    // The seeded README.md starts with `# Serper E2E Test Repo`, so the rich
    ...
    await expect(serperPage.getByRole('heading', { name: expectedHeading, level: 1 })).toBeVisible({
      timeout: 25_000
    })
  })
   ...
   */
  test('editor tab retains state when switching to terminal and back', async ({ serperPage }) => {
    const worktreeId = (await getActiveWorktreeId(serperPage))!
    await openFileExplorer(serperPage)
    ...
    const openFilesBefore = await getOpenFiles(serperPage, worktreeId)
    expect(openFilesBefore.length).toBeGreaterThan(0)

    const editorFileId = openFilesBefore[0].id

    // Switch to a terminal tab
    await switchToTerminal(serperPage, worktreeId)
    await expect
      .poll(async () => getActiveTabType(serperPage), { timeout: 3_000 })
      .not.toBe('editor')

    // Switch back to the same editor tab
    await switchToEditor(serperPage, editorFileId)
    await expect.poll(async () => getActiveTabType(serperPage), { timeout: 3_000 }).toBe('editor')

    // The same files should still be open
    const openFilesAfter = await getOpenFiles(serperPage, worktreeId)
