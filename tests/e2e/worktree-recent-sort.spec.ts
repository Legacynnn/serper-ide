...
test.describe('Worktree Recent Sort', () => {
  ...
  function createFolderFixture(): string {
    const dir = mkdtempSync(path.join(os.tmpdir(), 'serper-e2e-folder-'))
  ...
  test.afterEach(() => {
    ...
    // matches the mkdtempSync/rmSync pairing used in helpers/serper-app.ts
    // and helpers/serper-restart.ts.
  ...
  test('stamps lastActivityAt on a newly-added folder repo so it sorts to the top of Recent', async ({
    ...
    const repoId = await addFolderRepo(serperPage, folderPath)
    const lastActivityAt = await readFolderWorktreeLastActivity(serperPage, repoId)
  ...
  test('leaves lastActivityAt stable across repeated list refreshes', async ({ serperPage }) => {
    ...
    const first = await readFolderWorktreeLastActivity(serperPage, repoId)

    await serperPage.evaluate(async (id) => {
      await window.__store?.getState().fetchWorktrees(id)
      await window.__store?.getState().fetchWorktrees(id)
    }, repoId)

    const second = await readFolderWorktreeLastActivity(serperPage, repoId)
