...
test.describe('Windows terminal env and shell identity', () => {
  ...
  test('dev terminal preserves parent PATH so PATH commands resolve', async ({ serperPage }) => {
    await waitForActiveTerminalManager(serperPage)

    const ptyId = await waitForActivePanePtyId(serperPage)
    const marker = `__SERPER_E2E_NODE_PATH_${Date.now()}__`

    // Why: before the dev PATH fallback, daemon-spawned PTYs could get PATH set
    // to only Serper's dev CLI bin. A real terminal command catches that failure.
    await execInTerminal(serperPage, ptyId, `node -e "console.log('${marker}')"`)

    await waitForTerminalOutput(serperPage, marker, 15_000)
  })

  test('Windows tab icons stay pinned to the shell used at tab creation', async ({
    serperPage
  }) => {
    test.skip(process.platform !== 'win32', 'Windows shell icons only render on Windows')

    const tabIds = await serperPage.evaluate(() => {
    ...
    const tabSnapshot = await serperPage.evaluate(({ wslTabId, cmdTabId }) => {
    ...
    const wslTab = serperPage.locator(
      `[data-testid="sortable-tab"][data-tab-id="${tabIds.wslTabId}"]`
    )
    const cmdTab = serperPage.locator(
