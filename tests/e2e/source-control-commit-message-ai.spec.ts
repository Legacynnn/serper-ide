...
test.describe('Source Control AI commit messages', () => {
  test('generates a commit message from staged changes through the Source Control UI', async ({
    ...
    try {
      ...
      const textarea = serperPage.getByRole('textbox', { name: 'Commit message' })
      await expect(textarea).toBeVisible({ timeout: 10_000 })
      await expect(textarea).toHaveValue('')

      const generate = serperPage.getByRole('button', { name: 'Generate commit message with AI' })
      await expect(generate).toBeVisible()
      await expect(generate).toBeEnabled()
      await generate.click()

      await expect(
        serperPage.getByRole('button', { name: 'Stop generating commit message' })
      ).toBeVisible()
