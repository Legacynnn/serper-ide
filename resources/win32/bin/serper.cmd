...
REM launcher resolves APP_DIR back to resources/ and `serper open` cannot find
REM Serper.exe on packaged Windows installs.
for %%I in ("%RESOURCES_DIR%\..") do set "APP_DIR=%%~fI"
set "ELECTRON=%APP_DIR%\Serper.exe"

if not exist "%ELECTRON%" (
  echo Unable to locate Serper.exe next to "%RESOURCES_DIR%" 1>&2
  exit /b 1
)

REM Why: Serper packages the CLI entrypoint outside app.asar so the public shell
