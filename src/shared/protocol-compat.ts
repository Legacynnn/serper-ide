...
export function describeRuntimeCompatBlock(verdict: RuntimeCompatVerdict): string {
  ...
  if (verdict.reason === 'client-too-old') {
    return `This Serper client is too old for the selected server. Update Serper on this machine. Client protocol ${verdict.clientProtocolVersion}, server requires client protocol ${verdict.requiredClientProtocolVersion}.`
  }
  return `The selected Serper server is too old for this client. Update Serper on the server. Server protocol ${verdict.serverProtocolVersion}, client requires server protocol ${verdict.requiredServerProtocolVersion}.`
}
