...
// explicit launch/session facts Serper owns, not this inference path.
...
//     keyword land right after a path separator. Serper is a cross-platform
...
export function normalizeTerminalTitle(title: string): string {
  ...
  // churn while preserving the working/idle transition Serper keys off.
...
export function isClaudeAgent(title: string): boolean {
  ...
  if (containsBrailleSpinner(title)) {
    // Why: Serper synthesizes `⠋ Cursor Agent` working titles for cursor-agent
...
export function getAgentLabel(title: string): string | null {
  ...
  // (verified against the 2026.04.17 release) — Serper synthesizes the same
...
// across the entire turn — it carries zero working/idle information. Serper
...
export function detectAgentStatusFromTitle(title: string): AgentStatus | null {
  ...
  // action required") is either an Serper-synthesized working/permission title
