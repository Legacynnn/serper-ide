...
function baseSummary(overrides: Partial<HostedReviewQueueSummary> = {}): HostedReviewQueueSummary {
  return {
    identity: { provider: 'github', host: 'github.com', owner: 'acme', repo: 'serper', number: 42 },
...
describe('classifyHostedReview', () => {
  it('classifies mine/requested/agent/teammate', () => {
    ...
    expect(
      classifyHostedReview(baseSummary({ author: { login: 'serper-ci' } }), {
        agentAuthorLogins: ['serper-ci']
      }).state
    ).toBe('agent')
