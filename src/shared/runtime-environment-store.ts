...
const ENVIRONMENTS_FILE = 'serper-environments.json'
...
export function addEnvironmentFromPairingCode(
  ...
  if (!offer) {
    throw new RuntimeEnvironmentStoreError(
      'invalid_argument',
      'Invalid pairing code. Expected an serper://pair#... URL or bare pairing payload.'
    )
  }
...
function readEnvironmentStore(userDataPath: string): RuntimeEnvironmentStore {
  ...
  try {
    ...
    throw new RuntimeEnvironmentStoreError(
      'runtime_error',
      `Could not read Serper environments at ${path}; the file is invalid.`
    )
  }
}
