...
// the relay sends to Serper. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between Serper's main process and the remote relay.
//
// Per the design doc:
// - The relay normalizes; Serper routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. Serper's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is Serper's
//   local handle on an `ssh2` connection, not a wire identity. Serper stamps the
...
//   POST body so Serper's warn-once protocol diagnostics still fire. The relay
...
// with the same value so Serper can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → Serper. */
export type AgentHookRelayEnvelope = {
  source: AgentHookSource
  paneKey: string
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know Serper's local connectionId. */
  connectionId: null
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets Serper's warn-once
   ...
   *  Serper's `ingestRemote` validates it again at the SSH trust boundary. */
...
/** JSON-RPC request method Serper issues after `--connect` reattach to ask the
...
/** JSON-RPC request method Serper issues at session-ready to ship the
...
/** Feature-flag env var. Read once at process start by Serper and the relay.
 *  Remote agent hooks ship as the default SSH behavior; set "0" to opt out. */
export const SERPER_FEATURE_REMOTE_AGENT_HOOKS_ENV = 'SERPER_FEATURE_REMOTE_AGENT_HOOKS' as const

export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[SERPER_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
