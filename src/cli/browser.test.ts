...
describe('serper cli browser page targeting', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
  ...
  it('passes explicit page ids to snapshot without resolving the current worktree', async () => {
    ...
    await main(['snapshot', '--page', 'page-1', '--json'], '/tmp/not-an-serper-worktree')
  ...
  it('shows a single tab by page id', async () => {
    ...
    await main(['tab', 'show', '--page', 'page-1', '--json'], '/tmp/not-an-serper-worktree')
  ...
  it('resolves the current tab in a worktree', async () => {
    ...
    await main(['tab', 'current', '--worktree', 'all', '--json'], '/tmp/not-an-serper-worktree')
...
describe('serper cli browser tab profiles', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
...
describe('serper cli browser tab profiles', () => {
  ...
  it('lists browser tab profiles', async () => {
        ...
        profiles: [
          {
            ...
            scope: 'default',
            label: 'Default',
            partition: 'persist:serper-browser'
          },
          {
            id: 'work',
            scope: 'isolated',
            label: 'Work',
            partition: 'persist:serper-browser-session-work'
          }
        ]
      })
    )
    ...
    await main(['tab', 'profile', 'list', '--json'], '/tmp/not-an-serper-worktree')
  ...
  it('reports an empty browser tab profile list with a friendly message', async () => {
    ...
    await main(['tab', 'profile', 'list'], '/tmp/not-an-serper-worktree')
    ...
    queueFixtures(
      ...
      okFixture('req_profile_create', {
        profile: {
          ...
          scope: 'imported',
          label: 'From Chrome',
          partition: 'persist:serper-browser-session-imp'
        }
      })
    )
    vi.spyOn(console, 'log').mockImplementation(() => {})
  ...
  it('surfaces a runtime error if the registry refuses to create a profile', async () => {
    ...
    await main(['tab', 'profile', 'create', '--label', 'Bogus'], '/tmp/not-an-serper-worktree')
  ...
  it('reports a not-deleted profile in text mode without throwing', async () => {
    ...
    await main(['tab', 'profile', 'delete', '--profile', 'default'], '/tmp/not-an-serper-worktree')
...
describe('serper cli browser waits and viewport flags', () => {
  beforeEach(() => {
    callMock.mockReset()
  ...
  it('does not tell users Serper is down for a generic runtime timeout', async () => {
    callMock.mockRejectedValueOnce(
      new RuntimeClientError(
        'runtime_timeout',
        'Timed out waiting for the Serper runtime to respond.'
      )
    )
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})

    await main(['wait', '--selector', '#ready', '--worktree', 'all'], '/tmp/not-an-serper-worktree')

    expect(errorSpy).toHaveBeenCalledWith('Timed out waiting for the Serper runtime to respond.')
  })
