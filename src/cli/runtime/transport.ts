  ...
  return await new Promise((resolve, reject) => {
    ...
    if (!transport) {
      reject(
        new RuntimeClientError(
          'runtime_unavailable',
          'No compatible transport found in Serper runtime metadata.'
        )
      )
      return
    }
    ...
    socket.once('error', () => {
      finish({
        ...
        error: new RuntimeClientError(
          'runtime_unavailable',
          'Could not connect to the running Serper app. Restart Serper and try again.'
        )
      })
    })
    socket.on('data', (chunk) => {
      ...
      while (newlineIndex !== -1 && !settled) {
        ...
        try {
          ...
          finish({
            ...
            error: new RuntimeClientError(
              ...
              'The Serper runtime returned an invalid response frame.'
            )
          })
          return
        }
        ...
        // setTimeout().refresh() is stable since Node 10 (Serper ships on
        ...
        // the decode boundary so version skew between the CLI and the Serper
        ...
        if (response.id !== requestId) {
          finish({
            ...
            error: new RuntimeClientError(
              'invalid_runtime_response',
              'The Serper runtime returned a mismatched response id.'
            )
          })
          return
        }
        if (response._meta?.runtimeId && response._meta.runtimeId !== metadata.runtimeId) {
          finish({
            ...
            error: new RuntimeClientError(
              'runtime_unavailable',
              'The Serper runtime changed while the request was in flight. Retry the command.'
            )
          })
          return
        }
