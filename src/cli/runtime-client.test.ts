...
function writeMetadata(
  ...
  writeFileSync(
    join(userDataPath, 'serper-runtime.json'),
    JSON.stringify({
      runtimeId: 'runtime-1',
      pid,
      transports: [
...
describe.skipIf(process.platform === 'win32')('RuntimeClient', () => {
  it('returns the full RPC envelope for successful calls', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'serper-runtime-client-'))
  ...
  it('openSerper succeeds immediately when the runtime is already reachable', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'serper-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    const server = createServer((socket) => {
      sockets.add(socket)
    ...
    const client = new RuntimeClient(userDataPath, 100)
    const status = await client.openSerper(100)
