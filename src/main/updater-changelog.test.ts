...
function makeEntries(
  ...
  return items.map((item) => ({
    ...
    releaseNotesUrl: item.releaseNotesUrl ?? `https://onserper.dev/changelog/${item.version}`
  }))
}
...
describe('fetchChangelog', () => {
  ...
  it('returns exact match when the incoming version has rich content', async () => {
    const entries = makeEntries([
      {
        ...
        mediaUrl: 'https://onserper.dev/media/1.1.21.gif'
      },
      { version: '1.1.20' },
      { version: '1.1.19' }
    ])
    ...
    expect(result!.release.releaseNotesUrl).toBe('https://onserper.dev/changelog/1.1.21')
  ...
  it('falls back to the most recent rich entry when incoming version is not in JSON', async () => {
    ...
    const entries = makeEntries([
      {
        ...
        mediaUrl: 'https://onserper.dev/media/1.1.17.gif',
        releaseNotesUrl: 'https://onserper.dev/changelog/1.1.17'
      },
      { version: '1.1.16' },
      { version: '1.1.15' }
    ])
    ...
    expect(result!.release.releaseNotesUrl).toBe('https://onserper.dev/changelog')
    expect(result!.releasesBehind).toBe(2)
  })
  ...
  it('skips rich entries that the user has already passed', async () => {
    ...
    const entries = makeEntries([
      ...
      { version: '1.1.18' },
      {
        ...
        mediaUrl: 'https://onserper.dev/media/old.gif'
      }
    ])
    fetchMock.mockResolvedValue(jsonResponse(entries))
  ...
  it('shows rich entry at the same version as localVersion', async () => {
    ...
    const entries = makeEntries([
      { version: '1.1.20', description: '' },
      {
        ...
        mediaUrl: 'https://onserper.dev/media/current.gif'
      },
      { version: '1.1.17' }
    ])
    fetchMock.mockResolvedValue(jsonResponse(entries))
  ...
  it('shows rich entry when local version is not in JSON (very old user)', async () => {
    const entries = makeEntries([
      { version: '1.1.20', description: '' },
      {
        ...
        mediaUrl: 'https://onserper.dev/media/demo.gif'
      }
    ])
    fetchMock.mockResolvedValue(jsonResponse(entries))
    ...
    const entries = makeEntries([
      {
        ...
        mediaUrl: 'https://onserper.dev/media/latest.gif'
      },
      {
        version: '1.1.17',
