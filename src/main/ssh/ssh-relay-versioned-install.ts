...
// (~/.serper-remote/relay-v0.1.0) which the deploy step would overwrite in place
...
export function readLocalFullVersion(localRelayDir: string): string {
  ...
  if (!existsSync(versionFile)) {
    throw new Error(
      `Serper's local relay build is missing its version marker at ${versionFile}. ` +
        `This usually indicates a packaging or build problem; reinstall Serper.`
    )
  }
  const v = readFileSync(versionFile, 'utf-8').trim()
  if (!v) {
    throw new Error(
      `Serper's local relay version marker at ${versionFile} is empty. ` +
        `This usually indicates a packaging or build problem; reinstall Serper.`
    )
  }
  return v
