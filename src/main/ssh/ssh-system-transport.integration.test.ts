...
const sentinel = 'SERPER-RELAY v0.1.0 READY\\n';
...
function createRelayTree(root: string, remoteHome: string): void {
  ...
  const remoteDir = join(remoteHome, '.serper-remote', `relay-${RELAY_VERSION}`)
...
describe('system SSH transport integration', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join('/tmp', 'serper-ssh-'))
    oldHome = process.env.HOME
    oldRelayPath = process.env.SERPER_RELAY_PATH
    oldSystemSshPath = process.env.SERPER_SYSTEM_SSH_PATH
    oldForceSystemTransport = process.env.SERPER_SSH_FORCE_SYSTEM_TRANSPORT
    ...
    process.env.SERPER_RELAY_PATH = relayRoot
    process.env.SERPER_SYSTEM_SSH_PATH = writeFakeSsh(tempDir)
    process.env.SERPER_SSH_FORCE_SYSTEM_TRANSPORT = '1'
  })
  ...
  afterEach(() => {
    ...
    if (oldRelayPath === undefined) {
      delete process.env.SERPER_RELAY_PATH
    } else {
      process.env.SERPER_RELAY_PATH = oldRelayPath
    }
    if (oldSystemSshPath === undefined) {
      delete process.env.SERPER_SYSTEM_SSH_PATH
    } else {
      process.env.SERPER_SYSTEM_SSH_PATH = oldSystemSshPath
    }
    if (oldForceSystemTransport === undefined) {
      delete process.env.SERPER_SSH_FORCE_SYSTEM_TRANSPORT
    } else {
      process.env.SERPER_SSH_FORCE_SYSTEM_TRANSPORT = oldForceSystemTransport
    }
