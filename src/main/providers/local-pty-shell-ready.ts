 ...
 *
 * Why: when Serper needs to inject a startup command (e.g. issue command runner),
...
const SHELL_READY_MARKER = '\x1b]777;serper-shell-ready'
const SHELL_READY_MARKER_ESCAPED = '\\033]777;serper-shell-ready\\007'
...
function getShellReadyWrapperRoot(): string {
  const userDataPath = app?.getPath?.('userData') ?? process.env.SERPER_USER_DATA_PATH ?? tmpdir()
  return `${userDataPath}/shell-ready`
}

// Why: if our own process inherited ZDOTDIR from a parent shell that was
// itself an Serper PTY (e.g. the user launched `pn dev` from a terminal inside
// a running Serper), that ZDOTDIR points at an Serper shell-ready wrapper dir.
...
// (regardless of whether it came from this app's userData, a packaged Serper,
...
export function getBashShellReadyRcfileContent(): string {
  return `# Serper bash shell-ready wrapper
...
# PATH edits, hooks, and prompt init in Serper startup-command shells.
...
export function getZshShellReadyRcfileContent(): string {
  return `# Serper zsh shell-ready wrapper
...
function ensureShellReadyWrappers(): void {
  ...
  const zshEnv = `# Serper zsh shell-ready wrapper
...
`
  const zshProfile = `# Serper zsh shell-ready wrapper
_serper_home="\${SERPER_ORIG_ZDOTDIR:-$HOME}"
...
`
  ...
  const zshLogin = `# Serper zsh shell-ready wrapper
_serper_home="\${SERPER_ORIG_ZDOTDIR:-$HOME}"
case "\${_serper_home%/}" in
  */shell-ready/zsh) _serper_home="$HOME" ;;
esac
...
export function writeStartupCommandWhenShellReady(
  ...
  const flush = (): void => {
    ...
    // Why: run startup commands inside the same interactive shell Serper keeps
    ...
    // unsubmitted, forcing the user to press Enter after Serper launches the
