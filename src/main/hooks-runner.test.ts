...
describe('createSetupRunnerScript', () => {
  ...
  it('writes a fail-fast Windows runner that returns after batch commands', async () => {
    ...
    execFileSyncMock.mockReturnValue('C:\\repo\\.git\\worktrees\\feature\\serper\\setup-runner.cmd')
    ...
    try {
      ...
      expect(result).toEqual({
        runnerScriptPath: 'C:\\repo\\.git\\worktrees\\feature\\serper\\setup-runner.cmd',
        envVars: expect.objectContaining({
          SERPER_ROOT_PATH: '/test/repo',
          SERPER_WORKTREE_PATH: 'C:\\repo\\feature\\',
          SERPER_WORKSPACE_NAME: 'feature'
        })
      })
      expect(vi.mocked(fs.writeFileSync)).toHaveBeenCalledWith(
        'C:\\repo\\.git\\worktrees\\feature\\serper\\setup-runner.cmd',
        [
          '@echo off',
  ...
  it('derives SERPER_WORKSPACE_NAME from a POSIX worktree path', async () => {
    const originalPlatform = process.platform

    execFileSyncMock.mockReturnValue('/test/repo/.git/worktrees/feature/serper/setup-runner.sh')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'linux'
    })
    ...
    try {
      ...
      expect(result.envVars).toEqual(
        expect.objectContaining({
          SERPER_WORKTREE_PATH: '/test/repo-feature',
          SERPER_WORKSPACE_NAME: 'repo-feature'
        })
      )
    } finally {
      Object.defineProperty(process, 'platform', {
  ...
  it('translates WSL runner paths and env vars to Linux form on Windows', async () => {
    ...
    execFileSyncMock.mockReturnValue('/home/jin/.git/worktrees/feature/serper/setup-runner.sh')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    })
    ...
    try {
      ...
      expect(result).toEqual({
        runnerScriptPath:
          '\\\\wsl.localhost\\Ubuntu\\home\\jin\\.git\\worktrees\\feature\\serper\\setup-runner.sh',
        envVars: expect.objectContaining({
  ...
  it('translates WSL env vars to Linux paths when the worktree lives on a WSL UNC path', async () => {
    ...
    execFileSyncMock.mockReturnValue('/home/jin/repo/.git/worktrees/feature/serper/setup-runner.sh')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    })
    ...
    try {
      ...
      expect(result).toEqual({
        ...
        envVars: expect.objectContaining({
          SERPER_ROOT_PATH: '/test/repo',
          SERPER_WORKTREE_PATH: '/home/jin/repo/feature',
          SERPER_WORKSPACE_NAME: 'feature',
...
describe('createIssueCommandRunnerScript', () => {
  ...
  it('writes a POSIX runner under the worktree git dir for long issue commands', async () => {
    ...
    execFileSyncMock.mockReturnValue(
      '/test/repo/.git/worktrees/feature/serper/issue-command-runner.sh'
    )
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'linux'
    ...
    try {
      ...
      expect(result).toEqual({
        runnerScriptPath: '/test/repo/.git/worktrees/feature/serper/issue-command-runner.sh',
        envVars: expect.objectContaining({
          SERPER_ROOT_PATH: '/test/repo',
          SERPER_WORKTREE_PATH: '/test/repo-feature'
        })
