...
vi.mock('./updater-prerelease-feed', () => ({
  ...
  getReleaseDownloadUrl: (tag: string) =>
    `https://github.com/Legacynnn/serper/releases/download/${tag}`
}))
...
describe('updater', () => {
  ...
  it('opts into the RC channel when checkForUpdatesFromMenu is called with includePrerelease', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Legacynnn/serper/releases/download/v1.3.18-rc.1'
      })
  ...
  it('repins the generic feed to the newest RC tag for a prerelease user', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Legacynnn/serper/releases/download/v1.3.17-rc.2'
      })
      expect(autoUpdaterMock.checkForUpdates).toHaveBeenCalledTimes(1)
    })
  })
  ...
  it('repins the generic feed to a newer stable tag for a prerelease user', async () => {
    ...
    await vi.waitFor(() => {
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Legacynnn/serper/releases/download/v1.3.19'
      })
    })
    expect(autoUpdaterMock.allowPrerelease).not.toBe(true)
  })
  ...
  it('retries a prerelease check once against the previous feed tag when the manifest is missing', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenCalledWith({
        provider: 'generic',
        url: 'https://github.com/Legacynnn/serper/releases/download/v1.3.51-rc.7'
      })
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Legacynnn/serper/releases/download/v1.3.51-rc.6'
      })
    })
  ...
  it('pins the generic feed to a concrete stable tag for a stable user', async () => {
    ...
    expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
      provider: 'generic',
      url: 'https://github.com/Legacynnn/serper/releases/download/v1.3.18'
    })
  })
