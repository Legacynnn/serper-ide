...
describe('getWorkItemDetails PR file viewed state', () => {
  ...
  it('merges GitHub viewer viewed state into PR files', async () => {
    getWorkItemMock.mockResolvedValueOnce({
      ...
      title: 'Review files',
      state: 'open',
      url: 'https://github.com/Legacynnn/serper/pull/42',
      labels: [],
      updatedAt: '2026-04-01T00:00:00Z',
      author: null
    })
    getOwnerRepoMock.mockResolvedValue({ owner: 'Legacynnn', repo: 'serper' })
    ...
    ghExecFileAsyncMock.mockImplementation((args: string[]) => {
      ...
      if (endpoint === 'repos/Legacynnn/serper/pulls/42') {
        return Promise.resolve({
          stdout: JSON.stringify({
            body: 'PR body',
            head: { sha: 'head-sha' },
      ...
      if (endpoint === 'repos/Legacynnn/serper/pulls/42/files?per_page=100') {
        return Promise.resolve({
          stdout: JSON.stringify([
            {
