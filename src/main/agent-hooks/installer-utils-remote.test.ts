...
describe('installer-utils-remote', () => {
  ...
  it('writes the managed script and chmods 0o755', async () => {
    ...
    await writeManagedScriptRemote(
      sftp,
      '/home/u/.serper/agent-hooks/claude-hook.sh',
      '#!/bin/sh\n'
    )
    expect(fs.files.get('/home/u/.serper/agent-hooks/claude-hook.sh')).toBe('#!/bin/sh\n')
    expect(fs.modes.get('/home/u/.serper/agent-hooks/claude-hook.sh')).toBe(0o755)
  })
  ...
  it('replaces an existing managed script atomically via temp file rename', async () => {
    ...
    const path = '/home/u/.serper/agent-hooks/claude-hook.sh'
    ...
    expect(Array.from(fs.files.keys()).some((key) => key.includes('.serper-backup-'))).toBe(false)
  })
