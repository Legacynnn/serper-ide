...
// Why: this module is the Serper-main-process adapter for the shared
...
//     Serper restart so retained dashboard rows reappear on relaunch
...
import { SERPER_HOOK_PROTOCOL_VERSION } from '../../shared/agent-hook-types'
import {
...
// Why: name of the on-disk cache that survives Serper restart. Lives next to
...
export class AgentHookServer {
  ...
  private port = 0
  private token = ''
  // Why: identifies this Serper instance so hook scripts can stamp requests and
  ...
  private normalizeHookBodyPaneKeyAlias(body: unknown): unknown {
    ...
    // SERPER_PANE_KEY. The reattach path proves the UUID leaf once, then this
   ...
   *  identity Serper holds (the wire envelope carries connectionId: null and
   *  Serper stamps the real value here). The relay has already normalized the
  ...
  async start(options?: {
    ...
    this.server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
      ...
      if (req.headers['x-serper-agent-hook-token'] !== this.token) {
        res.writeHead(403)
        res.end()
        return
      }
  ...
  stop(): void {
    ...
    // would introduce a TOCTOU race vs. a concurrent Serper instance.
  ...
  buildPtyEnv(): Record<string, string> {
    ...
    const env: Record<string, string> = {
      SERPER_AGENT_HOOK_PORT: String(this.port),
      SERPER_AGENT_HOOK_TOKEN: this.token,
      SERPER_AGENT_HOOK_ENV: this.env,
      SERPER_AGENT_HOOK_VERSION: SERPER_HOOK_PROTOCOL_VERSION
    }
    ...
    if (this.endpointFileWritten && this.endpointFilePathCache) {
      env.SERPER_AGENT_HOOK_ENDPOINT = this.endpointFilePathCache
    }
    return env
  }
