...
describe('mapGhosttyToSerper — split-divider-color', () => {
  it('maps valid hex to both dark and light divider colors', () => {
    const result = mapGhosttyToSerper({ 'split-divider-color': '#ff5500' })
  ...
  it('rejects invalid split-divider-color', () => {
    const result = mapGhosttyToSerper({ 'split-divider-color': 'blue' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['split-divider-color'])
  })
})

describe('mapGhosttyToSerper — unfocused-split-opacity', () => {
  it('maps valid float to terminalInactivePaneOpacity', () => {
    const result = mapGhosttyToSerper({ 'unfocused-split-opacity': '0.5' })
    expect(result.diff).toEqual({ terminalInactivePaneOpacity: 0.5 })
    expect(result.unsupportedKeys).toEqual([])
  })

  it('rejects out-of-range unfocused-split-opacity', () => {
    const result = mapGhosttyToSerper({ 'unfocused-split-opacity': '1.2' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['unfocused-split-opacity'])
  })

  it('rejects negative unfocused-split-opacity', () => {
    const result = mapGhosttyToSerper({ 'unfocused-split-opacity': '-0.1' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['unfocused-split-opacity'])
  })
})

describe('mapGhosttyToSerper — scrollback-limit', () => {
  ...
  it('marks scrollback-limit as unsupported', () => {
    const result = mapGhosttyToSerper({ 'scrollback-limit': '50000' })
    expect(result.diff).toEqual({})
...
describe('mapGhosttyToSerper — cursor-text', () => {
  ...
  it('rejects invalid cursor-text', () => {
    const result = mapGhosttyToSerper({ 'cursor-text': 'white' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['cursor-text'])
  })
})

describe('mapGhosttyToSerper — bold-color', () => {
  it('maps valid hex to terminalColorOverrides.bold', () => {
    const result = mapGhosttyToSerper({ 'bold-color': '#ff0000' })
    expect(result.diff).toEqual({
