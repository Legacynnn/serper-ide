...
describe('Bitbucket hosted review integration', () => {
  beforeEach(() => {
    process.env = { ...OLD_ENV, SERPER_BITBUCKET_ACCESS_TOKEN: 'local-token' }
    delete process.env.SERPER_BITBUCKET_EMAIL
    delete process.env.SERPER_BITBUCKET_API_TOKEN
  ...
  it('resolves a Bitbucket PR through real git remote parsing and HTTP API calls', async () => {
    ...
    const repoPath = await mkdtemp(join(tmpdir(), 'serper-bitbucket-review-'))
    try {
      ...
      process.env.SERPER_BITBUCKET_API_BASE_URL = `http://127.0.0.1:${address.port}/2.0`
