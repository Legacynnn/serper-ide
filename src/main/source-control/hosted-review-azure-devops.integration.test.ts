...
describe('Azure DevOps hosted review integration', () => {
  beforeEach(() => {
    process.env = { ...OLD_ENV, SERPER_AZURE_DEVOPS_TOKEN: 'local-pat' }
    delete process.env.SERPER_AZURE_DEVOPS_API_BASE_URL
  ...
  it('resolves an Azure Repos PR through real git remote parsing and HTTP API calls', async () => {
    ...
    const repoPath = await mkdtemp(join(tmpdir(), 'serper-azure-review-'))
    try {
      const address = server.address()
      if (!address || typeof address === 'string') {
        throw new Error('expected TCP server address')
      }
      process.env.SERPER_AZURE_DEVOPS_API_BASE_URL = `http://127.0.0.1:${address.port}/acme/Project`
