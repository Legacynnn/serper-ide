...
const DEFAULT_MAC_COMMAND_PATH = '/usr/local/bin/serper'
...
export class CliInstaller {
  ...
  constructor(options: CliInstallerOptions = {}) {
    ...
    this.commandPathOverride =
      options.commandPathOverride ?? process.env.SERPER_CLI_INSTALL_PATH ?? null
    ...
    if (!launcherPath) {
      return {
        ...
        detail: this.isPackaged
          ? 'The bundled CLI launcher is missing from this Serper build.'
  ...
  async install(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'conflict') {
      throw new Error(`Refusing to replace non-Serper command at ${status.commandPath}.`)
    }
  ...
  async remove(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'conflict') {
      throw new Error(`Refusing to remove non-Serper command at ${status.commandPath}.`)
    }
    if (status.state === 'stale') {
      throw new Error(`Refusing to remove a command not owned by Serper at ${status.commandPath}.`)
    }
  ...
  private resolveCommandPath(): string | null {
    ...
    if (this.platform === 'linux') {
      ...
      return join(this.homePath, '.local', 'bin', 'serper')
    }

    if (this.platform === 'win32') {
      return join(this.localAppDataPath, 'Programs', 'Serper', 'bin', 'serper.cmd')
    }
    ...
    try {
      ...
      if (!stats.isSymbolicLink()) {
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not an Serper symlink.`
        })
      }
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Serper from the terminal.`
        })
      }
      throw error
    }
    ...
    try {
      ...
      if (!stats.isFile()) {
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not an Serper launcher script.`
        })
      }
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Serper from Command Prompt or PowerShell.`
        })
      }
      throw error
    }
...
async function ensureDevLauncher(args: {
  ...
  const launcherPath = join(
    args.userDataPath,
    ...DEV_LAUNCHER_DIR,
    args.platform === 'win32' ? 'serper.cmd' : 'serper'
  )
  await mkdir(dirname(launcherPath), { recursive: true })

  // Why: packaged Serper ships real platform launchers under resources/bin, but
...
set "SERPER_NODE_OPTIONS=%NODE_OPTIONS%"
set "SERPER_NODE_REPL_EXTERNAL_MODULE=%NODE_REPL_EXTERNAL_MODULE%"
set NODE_OPTIONS=
...
function buildWindowsForwarder(launcherPath: string): string {
  return `@echo off
setlocal
set "SERPER_LAUNCHER=${escapeWindowsBatchValue(launcherPath)}"
"%SERPER_LAUNCHER%" %*
`
}
...
async function writeWindowsUserPath(value: string): Promise<void> {
  await execFileAsync('powershell', [
    '-NoProfile',
    '-Command',
    // Why: PATH registration must stay user-scoped on Windows so the Serper
...
export function getBundledLauncherPath(
  ...
  if (platform === 'darwin' || platform === 'linux') {
    return join(resourcesPath, 'bin', 'serper')
  }
  if (platform === 'win32') {
    return join(resourcesPath, 'bin', 'serper.cmd')
  }
  return null
}
