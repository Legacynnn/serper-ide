...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(fixture.root, 'bin', 'serper')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: false,
        userDataPath: fixture.userDataPath,
        execPath: '/Applications/Serper.app/Contents/MacOS/Serper',
      ...
      expect(initial.launcherPath).toContain(join('userData', 'cli', 'bin', 'serper'))
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const fixture = await makeFixture()
      const installPath = join(fixture.root, '.local', 'bin', 'serper')
      const installer = new CliInstaller({
        platform: 'linux',
        isPackaged: false,
        userDataPath: fixture.userDataPath,
        execPath: '/opt/Serper/serper',
        appPath: fixture.appPath,
        commandPathOverride: installPath,
  ...
  it('creates a windows wrapper and updates the user PATH', async () => {
    const fixture = await makeFixture()
    const installPath = join(fixture.root, 'Programs', 'Serper', 'bin', 'serper.cmd')
    let userPath = 'C:\\Windows\\System32'
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      userDataPath: fixture.userDataPath,
      execPath: 'C:\\Users\\me\\AppData\\Local\\Serper\\Serper.exe',
      appPath: fixture.appPath,
      commandPathOverride: installPath,
    ...
    expect(installed.state).toBe('installed')
    expect(installed.pathConfigured).toBe(true)
    expect(userPath).toContain(join(fixture.root, 'Programs', 'Serper', 'bin'))

    const wrapperContent = await readFile(installPath, 'utf8')
    expect(wrapperContent).toContain('SERPER_LAUNCHER=')
    expect(wrapperContent).toContain('serper.cmd')

    const removed = await installer.remove()
    expect(removed.state).toBe('not_installed')
    expect(userPath).not.toContain(join(fixture.root, 'Programs', 'Serper', 'bin'))
  })

  // Why: this test creates a Unix symlink to /tmp/not-serper, which only applies on macOS/Linux.
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      await symlink('/tmp/not-serper', installPath)
