...
describe('resolveConsent', () => {
  ...
  it('returns serper_disabled when SERPER_TELEMETRY_DISABLED=1', () => {
    process.env.SERPER_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
        settingsWithTelemetry({
          optedIn: true,
  ...
  it('prefers do_not_track over serper_disabled when both are set', () => {
    process.env.DO_NOT_TRACK = '1'
    process.env.SERPER_TELEMETRY_DISABLED = '1'
    expect(
      resolveConsent(
  ...
  it('env-var override wins over a stored pending_banner state', () => {
    ...
    ).toEqual({ effective: 'disabled', reason: 'serper_disabled' })
  })
})
