...
// `which` probe even though they work fine from Terminal (see Legacynnn/serper#829).
//
...
const DELIMITER = '__SERPER_SHELL_PATH__'
...
function spawnShellAndReadPath(shell: string): Promise<HydrationResult> {
  return new Promise((resolve) => {
    ...
    const child = spawn(shell, ['-ilc', command], {
      ...
      // and we don't want that in Serper's console.
      env: process.env,
