...
const BASE_APP_NAME = 'Serper'
const BASE_APP_USER_MODEL_ID = 'com.legacynnn.serper'
...
export function getDevInstanceIdentity(
  ...
  const repoRoot = cleanEnvValue(env.SERPER_DEV_REPO_ROOT)
  const branch = cleanEnvValue(env.SERPER_DEV_BRANCH)
  const worktreeName =
    cleanEnvValue(env.SERPER_DEV_WORKTREE_NAME) ??
    cleanEnvValue(path.basename(repoRoot ?? process.cwd()))
  const devLabel = cleanEnvValue(env.SERPER_DEV_INSTANCE_LABEL) ?? formatLabel(branch, worktreeName)
  const dockTitle =
    cleanEnvValue(env.SERPER_DEV_DOCK_TITLE) ?? `${BASE_APP_NAME}: ${branch ?? devLabel ?? 'dev'}`
