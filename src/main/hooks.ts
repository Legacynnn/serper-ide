...
/**
 * Parse a simple serper.yaml file. Handles only the supported `scripts:` and
 * `issueCommand:` keys with multiline string values (YAML block scalar `|`).
 */
export function parseSerperYaml(content: string): SerperHooks | null {
  const hooks: SerperHooks = { scripts: {} }
  ...
  for (const line of lines) {
    ...
    if (currentSection === 'issueCommand' && line.startsWith('  ')) {
      // Why: `issueCommand` is a top-level scalar in `serper.yaml`, so its block
...
/**
 * Load hooks from serper.yaml in the given repo root.
 */
export function loadHooks(repoPath: string): SerperHooks | null {
  const yamlPath = join(repoPath, 'serper.yaml')
  ...
  try {
    ...
    return parseSerperYaml(content)
  } catch {
    return null
  }
}
...
/**
 * Check whether an serper.yaml exists for a repo.
 */
export function hasHooksFile(repoPath: string): boolean {
  return existsSync(join(repoPath, 'serper.yaml'))
}

// Why: when a newer Serper release adds a top-level key to `serper.yaml` (like
// `issueCommand` was added here), older versions that don't recognise it will
// return `null` from `parseSerperYaml` and show a confusing "could not be parsed"
// error.  Detecting well-formed but unrecognised keys lets the UI suggest an
// update instead of implying the file is broken.
const RECOGNIZED_SERPER_YAML_KEYS = new Set(['scripts', 'issueCommand'])

/**
 * Return true when `serper.yaml` contains at least one top-level key that this
 * version of Serper does not handle.
 */
export function hasUnrecognizedSerperYamlKeys(repoPath: string): boolean {
  try {
    const content = readFileSync(join(repoPath, 'serper.yaml'), 'utf-8')
    return content.split(/\r?\n/).some((line) => {
      ...
      return m != null && !RECOGNIZED_SERPER_YAML_KEYS.has(m[1])
    })
  } catch {
    return false
  }
...
// ─── Issue command files ────────────────────────────────────────────────
// Why: `serper.yaml` is the tracked, project-wide defaults surface, while
// `.serper/issue-command` remains the per-user override. Keeping the local file in
// `.serper/` lets users customize agent automation without editing committed config.

const SERPER_DIR = '.serper'
const ISSUE_COMMAND_FILENAME = 'issue-command'

export function getIssueCommandFilePath(repoPath: string): string {
  return join(repoPath, SERPER_DIR, ISSUE_COMMAND_FILENAME)
}
...
/**
 * Write the per-user issue command override to `{repoRoot}/.serper/issue-command`.
 * Creates `.serper/` and ensures it is in `.gitignore` on first write.
 * If content is empty, deletes only the override so the shared `serper.yaml`
...
export function writeIssueCommand(repoPath: string, content: string): void {
  ...
  try {
    ...
    const serperDir = join(repoPath, SERPER_DIR)
    if (!existsSync(serperDir)) {
      mkdirSync(serperDir, { recursive: true })
    }
    ensureSerperDirIgnored(repoPath)
...
/**
 * Ensure `.serper` is listed in the repo's `.gitignore` so the per-user
 * directory is never accidentally committed.
 */
function ensureSerperDirIgnored(repoPath: string): void {
  const gitignorePath = join(repoPath, '.gitignore')
  try {
    if (existsSync(gitignorePath)) {
      const content = readFileSync(gitignorePath, 'utf-8')
      if (/^\.serper\/?$/m.test(content)) {
        return
      }
      const separator = content.endsWith('\n') ? '' : '\n'
      writeFileSync(gitignorePath, `${content}${separator}.serper\n`, 'utf-8')
    } else {
      writeFileSync(gitignorePath, '.serper\n', 'utf-8')
    }
  } catch {
    console.warn('[hooks] Could not update .gitignore to exclude .serper')
  }
}
...
function getEffectiveHookScript(
  ...
  // 'shared-only' would silently drop those scripts when serper.yaml is missing.
  if (legacyFallback) {
...
export function getEffectiveHooks(repo: Repo, worktreePath?: string): SerperHooks | null {
  ...
  // Why: committed `serper.yaml` and local Settings commands can intentionally
...
export function getSetupCommandSource(
  ...
  // serper.yaml is missing; otherwise upgrading silently drops the legacy hook.
...
function getSetupEnvVars(repo: Repo, worktreePath: string): Record<string, string> {
  return {
    SERPER_ROOT_PATH: repo.path,
    SERPER_WORKTREE_PATH: worktreePath,
    SERPER_WORKSPACE_NAME: getRuntimePathBasename(worktreePath),
...
function createWorktreeRunnerScript(
  ...
  const gitRelPath = useWindowsFormat
    ? `serper/${runnerBaseName}.cmd`
    : `serper/${runnerBaseName}.sh`
  ...
  // Why: when the worktree is on WSL, env vars like SERPER_ROOT_PATH and
  // SERPER_WORKTREE_PATH contain Windows UNC paths. The setup script runs
...
export function runHook(
  ...
  if (wslInfo) {
    ...
    // Why: translate SERPER_ROOT_PATH / SERPER_WORKTREE_PATH to Linux paths so
    // hook scripts that reference $SERPER_WORKTREE_PATH get usable paths
