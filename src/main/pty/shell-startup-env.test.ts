...
describe('readShellStartupEnvVar', () => {
  ...
  // Issue #1534 / PR description: GUI-launched Serper does not inherit
  ...
  it('mirrors the user scenario: GUI-launched Serper discovers .zshrc-only export', () => {
    mockStartupFiles({
  ...
  it('does not scan .bashrc for bash shells', () => {
    // Why: Serper launches bash as a login shell and the shell-ready wrappers
    // intentionally do NOT source .bashrc, so a value present only in .bashrc
    // would never be set in the live Serper bash shell.
    process.env.SHELL = '/bin/bash'
  ...
  it('honors an explicit shell argument over process.env.SHELL', () => {
    ...
    // differs from the Serper process's own $SHELL.
    process.env.SHELL = '/bin/zsh'
    mockStartupFiles({
      '.zshrc': 'export OPENCODE_CONFIG_DIR=/from/zshrc\n',
      '.bash_profile': 'export OPENCODE_CONFIG_DIR=/from/bash_profile\n'
