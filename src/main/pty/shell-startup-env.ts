...
// Why: Serper launches bash as a login shell (see local-pty-shell-ready.ts
...
// .bashrc. Scanning .bashrc would mirror values the live Serper bash never sees.
...
function shellStartupFilePaths(home: string, shell: string | undefined): readonly string[] {
  if (!shell) {
    // Why: Serper's POSIX default shell is /bin/zsh when $SHELL is unset.
  ...
  // use Serper's zsh/bash shell-ready startup files, so scanning those files
 ...
 *
 * Why: GUI-launched Serper does not inherit interactive shell exports, but the
