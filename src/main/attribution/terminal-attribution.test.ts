...
describe('applyTerminalAttributionEnv', () => {
  ...
  function makeTmpRoot(): string {
    tmpRoot = mkdtempSync(join(tmpdir(), 'serper-attribution-'))
    return tmpRoot
  }
  ...
  function stripInheritedAttributionPath(pathValue: string): string {
    ...
    return pathValue
      .split(pathDelimiter)
      .filter((entry) => !entry.includes('serper-terminal-attribution'))
      .join(pathDelimiter)
  }

  function cleanAttributionEnv(env?: Record<string, string>): Record<string, string> {
    const base = { ...process.env }
    delete base.SERPER_ENABLE_GIT_ATTRIBUTION
    delete base.SERPER_GIT_COMMIT_TRAILER
    delete base.SERPER_GH_PR_FOOTER
    delete base.SERPER_GH_ISSUE_FOOTER
    delete base.SERPER_ATTRIBUTION_SHIM_DIR
    delete base.SERPER_REAL_GIT
    delete base.SERPER_REAL_GH
  ...
  it('does not amend HEAD when git commit --dry-run exits successfully', () => {
    ...
    mkdirSync(repo)
    runGit(repo, ['init'])
    runGit(repo, ['config', 'user.name', 'Serper Test'])
    runGit(repo, ['config', 'user.email', 'serper-test@example.com'])
    ...
    expect(runGit(repo, ['log', '-1', '--format=%B'])).not.toContain('Co-authored-by: Serper')
    ...
    expect(runGit(repo, ['log', '-1', '--format=%B'])).toContain(
      'Co-authored-by: Serper <help@stably.ai>'
    )
  })
...
`,
    ...
    expect(readFileSync(argsPath, 'utf8')).not.toContain('Co-authored-by: Serper')
  })
...
grep -Fq 'Co-authored-by: Serper <help@stably.ai>' "$1"
`,
      'utf8'
    )
...
`,
    ...
    expect(readFileSync(argsPath, 'utf8')).toContain('Co-authored-by: Serper <help@stably.ai>')
  })
...
if [[ "$1 $2 $3 $4" == "pr view --json url" ]]; then
  printf '%s\\n' 'https://github.com/Legacynnn/serper/pull/123'
  exit 0
fi
if [[ "$1 $2" == "api repos/Legacynnn/serper/pulls/123" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'Existing body'
  exit 0
fi
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/Legacynnn/serper/pulls/123" ]]; then
  touch "${markerPath}"
  exit 0
fi
exit 1
...
if [[ "$1 $2" == "issue create" ]]; then
  printf '%s\\n' 'https://github.com/Legacynnn/serper/issues/456'
  exit 0
fi
if [[ "$1 $2" == "api repos/Legacynnn/serper/pulls/123" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'PR body'
...
if [[ "$1 $2" == "api repos/Legacynnn/serper/issues/456" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'Issue body'
  exit 0
fi
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/Legacynnn/serper/pulls/123" ]]; then
...
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/Legacynnn/serper/issues/456" ]]; then
...
`,
    ...
    ).toBe('https://github.com/Legacynnn/serper/pull/123\n')
    expect(
      execFileSync('gh', ['issue', 'create', '--title', 'Issue', '--body', 'Body'], {
        encoding: 'utf8',
        env: cleanAttributionEnv(attributionEnv)
      })
    ).toBe('https://github.com/Legacynnn/serper/issues/456\n')
...
`,
    ...
    expect(output).toBe('https://github.com/Legacynnn/serper/pull/123\n')
    expect(existsSync(markerPath)).toBe(false)
  })
...
`,
  ...
  it('fails open when shim files cannot be written', () => {
    ...
    expect(baseEnv.SERPER_ENABLE_GIT_ATTRIBUTION).toBeUndefined()
  ...
  it('does not duplicate shim directories when applied to an already-injected env', () => {
    ...
    const shimEntries = baseEnv.PATH.split(pathDelimiter).filter((entry) =>
      entry.includes('serper-terminal-attribution')
    )
  ...
  it('writes PowerShell wrappers without raw-template backslash escapes', () => {
    ...
    const shimDir = join(root, 'user-data', 'serper-terminal-attribution', 'win32')
