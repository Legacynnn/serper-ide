...
// Why: the dispatch preamble teaches agents about Serper's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use serper-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong Serper instance (Section 6.4).
  const cli = params.devMode ? 'serper-dev' : 'serper'

  const header = `You are working inside Serper, a multi-agent IDE. You are a dispatched worker.
