      ...
      methods: [
        defineStreamingMethod({
          ...
          handler: async (params, { runtime }, emit) => {
            const read = await (runtime as SerperRuntimeService).readTerminal(params.terminal)
            emit({ type: 'scrollback', lines: read.tail, truncated: read.truncated })

            const leaf = (runtime as SerperRuntimeService).resolveLeafForHandle(params.terminal)
            if (!leaf?.ptyId) {
              emit({ type: 'end' })
            }
          }
      ...
      methods: [
        defineStreamingMethod({
          ...
          handler: async (_params, { runtime }, emit) => {
            ...
            await new Promise<void>((resolve) => {
              ;(runtime as SerperRuntimeService).registerSubscriptionCleanup('sub-1', () => {
                emit({ type: 'end' })
                resolve()
              })
            })
        ...
        defineMethod({
          ...
          handler: async (params, { runtime }) => {
            ;(runtime as SerperRuntimeService).cleanupSubscription(params.subscriptionId)
