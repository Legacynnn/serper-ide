...
const HERMES_PLUGIN_NAME = 'serper-status'
const HERMES_PLUGIN_MARKER = 'Managed by Serper. Do not edit; changes may be overwritten.'
...
function enablePlugin(config: HermesConfig): HermesConfig {
  ...
  if (disabled === null) {
    ...
    // so Serper's install status matches what the real Hermes loader will do.
    plugins.disabled = []
...
function getPluginFilesState(pluginDir = getPluginDir()): {
  ...
  try {
    ...
    return {
      present: true,
      managed,
      detail: managed ? null : 'Hermes serper-status plugin exists but is not Serper-managed'
    }
  } catch (error) {
    return {
      present: true,
...
function buildStatus(configPath: string, config: HermesConfig): AgentHookInstallStatus {
  ...
  const details = [
    pluginFiles.detail,
    enablement.detail,
    !enablement.enabled ? 'serper-status is not enabled in Hermes config.yaml' : null,
    enablement.disabled ? 'serper-status is disabled in Hermes config.yaml' : null
...
function getPluginManifest(): string {
  return [
    ...
    'version: 1.0.0',
    'description: "Reports Hermes Agent lifecycle events to Serper."',
    'author: "Serper"',
    'kind: standalone',
    'provides_hooks:',
...
def _endpoint_env() -> dict[str, str]:
    env = dict(os.environ)
    endpoint = env.get("SERPER_AGENT_HOOK_ENDPOINT", "")
...
def _post_to_serper(payload: dict[str, Any]) -> None:
    env = _endpoint_env()
    port = env.get("SERPER_AGENT_HOOK_PORT", "")
    token = env.get("SERPER_AGENT_HOOK_TOKEN", "")
    pane_key = env.get("SERPER_PANE_KEY", "")
    if not port or not token or not pane_key:
        return
    body = {
        "paneKey": pane_key,
        "tabId": env.get("SERPER_TAB_ID", ""),
        "worktreeId": env.get("SERPER_WORKTREE_ID", ""),
        "env": env.get("SERPER_AGENT_HOOK_ENV", ""),
        "version": env.get("SERPER_AGENT_HOOK_VERSION", ""),
        "payload": payload,
    }
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "X-Serper-Agent-Hook-Token": token,
        },
    )
    try:
...
def _make_hook(event_name: str) -> Callable[..., None]:
    def _hook(**kwargs: Any) -> None:
        _post_to_serper(_payload_for_event(event_name, kwargs))
