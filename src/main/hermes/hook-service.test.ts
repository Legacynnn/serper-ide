...
describe('HermesHookService', () => {
  ...
  beforeEach(() => {
    ...
    homeDir = mkdtempSync(join(tmpdir(), 'serper-hermes-hooks-'))
  ...
  it('preserves other enabled plugins and removes Serper from disabled list', () => {
    writeFileSync(
  ...
  it('registered plugin hooks post normalized JSON to Serper', async () => {
    ...
    const received = new Promise<Record<string, unknown>>((resolve, reject) => {
      const server = createServer((req, res) => {
        ...
        req.on('end', () => {
          try {
            ...
            expect(req.headers['x-serper-agent-hook-token']).toBe('token-1')
            res.writeHead(204)
            res.end()
      ...
      server.listen(0, '127.0.0.1', () => {
        ...
        const script = [
          ...
          `spec = importlib.util.spec_from_file_location("serper_status", ${JSON.stringify(initPath)})`,
        ...
        execFile(
          ...
          ['-c', script],
          {
            env: {
              ...process.env,
              SERPER_AGENT_HOOK_PORT: String(address.port),
              SERPER_AGENT_HOOK_TOKEN: 'token-1',
              SERPER_AGENT_HOOK_ENDPOINT: '',
              SERPER_PANE_KEY: PANE_KEY,
              SERPER_TAB_ID: 'tab-1',
