...
const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  var g = window.__serperGrab;
  if (g && g.host) g.host.style.display = 'none';
  document.querySelectorAll('[data-serper-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-serper-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-serper-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-serper-previous-display') || '';
    el.removeAttribute('data-serper-previous-display');
  });
})()`
