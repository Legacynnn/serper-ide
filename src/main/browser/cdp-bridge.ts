...
export class CdpBridge {
  ...
  getPageInfo(
    ...
    // Why: SerperRuntimeService pushes navigation/title updates after commands
  ...
  private getActiveGuest(): Electron.WebContents {
    ...
    if (tabs.length === 0) {
      throw new BrowserError(
        'browser_no_tab',
        'No browser tab is open. Use the Serper UI to open a browser tab first.'
      )
    }
    if (tabs.length === 1) {
      ...
      throw new BrowserError(
        'browser_no_tab',
        "Multiple browser tabs are open. Run 'serper tab list' and 'serper tab switch --index <n>' to select one."
      )
    }
    ...
    if (!guest || guest.isDestroyed()) {
      ...
      throw new BrowserError(
        ...
        "The active browser tab was closed. Run 'serper tab list' to find remaining tabs."
      )
    }
    return guest
  }
  ...
  private async resolveRef(
    ...
    if (!state.snapshotResult) {
      throw new BrowserError(
        'browser_stale_ref',
        "No snapshot exists for this tab. Run 'serper snapshot' first."
      )
    }
    ...
    if (!entry) {
      throw new BrowserError(
        ...
        `Element ref ${ref} was not found. Run 'serper snapshot' to see available refs.`
      )
    }
    ...
    if (!entry.sessionId) {
      ...
      if (state.navigationId && currentNavId !== state.navigationId) {
        ...
        throw new BrowserError(
          'browser_stale_ref',
          "The page has navigated since the last snapshot. Run 'serper snapshot' to get fresh refs."
        )
      }
    }
    ...
    try {
      ...
      throw new BrowserError(
        'browser_stale_ref',
        `Element ${ref} no longer exists in the DOM. Run 'serper snapshot' to get fresh refs.`
      )
    }
  }
