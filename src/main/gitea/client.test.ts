...
describe('Gitea client', () => {
  beforeEach(() => {
    process.env = { ...OLD_ENV }
    process.env.SERPER_GITEA_TOKEN = 'gitea-token'
    delete process.env.SERPER_GITEA_API_BASE_URL
  ...
  it('uses an API base URL override for subpath or non-standard deployments', async () => {
    process.env.SERPER_GITEA_API_BASE_URL = 'https://git.example.com/code'
  ...
  it('verifies token auth when a global API base URL is configured', async () => {
    process.env.SERPER_GITEA_API_BASE_URL = 'https://git.example.com'
