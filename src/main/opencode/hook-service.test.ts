...
describe('OpenCode hook plugin source', () => {
  ...
  it('still accepts an optional opaque plugin context instead of destructuring', () => {
    ...
    expect(source).toContain('export const SerperOpenCodeStatusPlugin = async (_ctx) => {')
  ...
  it('resolves hook coords from the endpoint file before falling back to process.env', () => {
    // Why: a long-running OpenCode session was fork()ed with the prior Serper's
    // PORT/TOKEN frozen into process.env. The plugin must prefer the on-disk
    // endpoint file (rewritten on every Serper start()) over env, otherwise it
    // keeps posting to a dead port after an Serper restart.
    const source = _internals.getOpenCodePluginSource()

    expect(source).toContain('function readEndpointFile()')
    expect(source).toContain('process.env.SERPER_AGENT_HOOK_ENDPOINT')
    ...
    expect(source).toContain('"X-Serper-Agent-Hook-Token": coords.token')
  })
...
describe('OpenCodeHookService buildPtyEnv / clearPty round-trip', () => {
  ...
  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'serper-opencode-hooks-'))
  ...
  it('writes OPENCODE_CONFIG_DIR for a daemon-shaped sessionId and installs the plugin file', () => {
    ...
    const pluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'serper-opencode-status.js')
    expect(existsSync(pluginPath)).toBe(true)
    // Sanity-check the file has plugin source, not a stray write.
    expect(readFileSync(pluginPath, 'utf8')).toContain('SerperOpenCodeStatusPlugin')
  })
  ...
  it('buildPtyEnv preserves a user-set OPENCODE_CONFIG_DIR when the id is unusable', () => {
    ...
    const service = new OpenCodeHookService()
    const userDir = mkdtempSync(join(tmpdir(), 'serper-opencode-userdir-'))
    try {
  ...
  it('works end-to-end for a plain UUID id (non-daemon path)', () => {
    ...
    expect(existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'serper-opencode-status.js'))).toBe(
      true
    )
...
describe('OpenCodeHookService overlay mode (user OPENCODE_CONFIG_DIR set)', () => {
  ...
  // Serper must mirror it into a per-PTY overlay rather than `delete` its own
  // injection (the prior bug) or overwrite the user's value (Superset's
  // failure mode). The user's auth/models/keymap and Serper's status plugin
  ...
  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'serper-opencode-overlay-userdata-'))
    getPathMock.mockImplementation((name: string) => {
      if (name === 'userData') {
        return userDataDir
      }
  ...
  beforeEach(() => {
    userConfigDir = mkdtempSync(join(tmpdir(), 'serper-opencode-overlay-userconfig-'))
  ...
  it('builds an overlay under userData and exposes user config + Serper plugin together', () => {
    const service = new OpenCodeHookService()
    ...
    // Serper's status plugin is a sibling, not a replacement.
    const serperPluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'serper-opencode-status.js')
    expect(existsSync(serperPluginPath)).toBe(true)
    expect(readFileSync(serperPluginPath, 'utf8')).toContain('SerperOpenCodeStatusPlugin')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: only the plugins/ subtree needs entry-by-entry mirroring so Serper
      ...
      // plugins/ must be a real directory in the overlay so Serper can write
  ...
  it("does not overwrite a user plugin file with the same filename as Serper's plugin", () => {
    // Why: the failure mode this guards against — a user-owned plugin file
    // happens to be named serper-opencode-status.js. Without the per-entry
    // skip in mirrorUserConfig, the file would be linked into the overlay
    // and Serper's writeFileSync would write through the symlink, destroying
    // the user's content on their real filesystem.
    const userSerperSentinel = 'USER OWNED SERPER-NAMED PLUGIN — DO NOT CLOBBER'
    writeFileSync(join(userConfigDir, 'plugins', 'serper-opencode-status.js'), userSerperSentinel)
    ...
    // Overlay copy is Serper's real plugin source, not the user's file.
    const overlayPlugin = readFileSync(
      join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'serper-opencode-status.js'),
      'utf8'
    )
    expect(overlayPlugin).toContain('SerperOpenCodeStatusPlugin')
    expect(overlayPlugin).not.toBe(userSerperSentinel)
    expectUserConfigIntact()
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: if plugins/ is a symlink (common dotfiles pattern), writing Serper's
      ...
      const realPluginsDir = mkdtempSync(join(tmpdir(), 'serper-real-plugins-'))
      try {
        ...
        // The user's real filesystem must NOT receive Serper's status plugin.
        expect(existsSync(join(realPluginsDir, 'serper-opencode-status.js'))).toBe(false)
        ...
        // Serper's status plugin lands in the overlay only.
        expect(
          existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'serper-opencode-status.js'))
        ).toBe(true)
  ...
  it("preserves the user's OPENCODE_CONFIG_DIR when the path does not exist", () => {
    // Why: typoed user path — overriding it with an Serper-owned dir would let
    // Serper's status plugin "succeed" while silently hiding the user's typo.
    // The design rejects that: leave the user's value alone and let OpenCode
    // surface the typo on its own.
    const service = new OpenCodeHookService()
    const missingPath = join(tmpdir(), `serper-opencode-nope-${Date.now()}`)
  ...
  it("preserves the user's OPENCODE_CONFIG_DIR when the mirror step fails", async () => {
    ...
    // the user's plugins/auth/models keep loading; only Serper's status plugin
    // is forfeited.
  ...
  it('rebuilding the overlay for the same ptyId does not corrupt the user dir', () => {
    ...
    // overlay with both user files and Serper's plugin.
    const service = new OpenCodeHookService()
    service.buildPtyEnv(ptyId, userConfigDir)
    service.buildPtyEnv(ptyId, userConfigDir)
    const env = service.buildPtyEnv(ptyId, userConfigDir)
    ...
    expect(
      readFileSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'serper-opencode-status.js'), 'utf8')
    ).toContain('SerperOpenCodeStatusPlugin')
    expectUserConfigIntact()
  })
})
