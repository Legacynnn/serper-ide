...
describe('OpenCodeUsageStore', () => {
  ...
  it('reports no data for Serper scope when only non-Serper OpenCode usage exists', async () => {
  ...
  it('uses recorded OpenCode costs and token totals without model pricing inference', async () => {
    ...
    const summary = await store.getSummary('serper', '30d')
    const daily = await store.getDaily('serper', '30d')
  ...
  it('returns recent sessions with OpenCode event and token fields', async () => {
    ...
    const sessions = await store.getRecentSessions('serper', '30d', 5)
