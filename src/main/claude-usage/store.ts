...
export function initClaudeUsagePath(): void {
  _claudeUsageFile = join(app.getPath('userData'), 'serper-claude-usage.json')
}
...
export class ClaudeUsageStore {
  ...
  private load(): ClaudeUsagePersistedState {
    try {
      ...
      // preventing Serper from booting, but we leave the file on disk for debugging.
  ...
  async getSummary(scope: ClaudeUsageScope, range: ClaudeUsageRange): Promise<ClaudeUsageSummary> {
    ...
    return {
      ...
      // data here makes the Serper-only view render empty charts instead of the
      // intended "no usage for this scope" message when only off-Serper logs exist.
  ...
  async getAutomationRunUsage(input: AutomationUsageLookupInput): Promise<AutomationRunUsage> {
    ...
    return {
      ...
      // Why: Serper terminal tab ids and Claude usage session ids are different
  ...
  private getFilteredDaily(scope: ClaudeUsageScope, range: ClaudeUsageRange) {
    ...
    return this.state.dailyAggregates.filter((entry) => {
      ...
      if (scope === 'serper' && entry.worktreeId === null) {
        return false
      }
      return true
    })
