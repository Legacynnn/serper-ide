...
export const createEditorSlice: StateCreator<AppState, [], [], EditorSlice> = (set, get) => ({
  ...
  closeFile: (fileId) => {
    ...
    set((s) => {
      ...
      return {
        ...
        // close handlers and return to the Serper landing state instead of
  ...
  beginRemoteOperation: (kind) =>
    set((s) => ({
      ...
      // so a second remote op can't be started from inside Serper. If a
  ...
  activateMarkdownLink: async (rawHref, ctx) => {
    ...
    if (target.kind === 'file') {
      ...
      if (target.relativePath === undefined) {
        ...
        // before opening them in Serper. Markdown file:// links need the same
      ...
      for (const [worktreeId, files] of Object.entries(openFilesByWorktree)) {
        ...
        for (const pf of files) {
          openFiles.push({
            ...
            worktreeId,
            // Why: sessions can contain language ids from older Serper builds.
