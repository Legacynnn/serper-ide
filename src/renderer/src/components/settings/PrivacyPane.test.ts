...
describe('PrivacyPane — isEnvBlocked', () => {
  ...
  it('returns true for SERPER_TELEMETRY_DISABLED', () => {
    expect(isEnvBlocked({ effective: 'disabled', reason: 'serper_disabled' })).toBe(true)
  })
...
describe('PrivacyPane — envVarNameForReason', () => {
  ...
  it('maps serper_disabled to SERPER_TELEMETRY_DISABLED', () => {
    expect(envVarNameForReason('serper_disabled')).toBe('SERPER_TELEMETRY_DISABLED')
  })
...
describe('PrivacyPane — computeBlockedReason', () => {
  ...
  it('names SERPER_TELEMETRY_DISABLED as the env reason when set', () => {
    const result = computeBlockedReason({ effective: 'disabled', reason: 'serper_disabled' })
    expect(result).toEqual({ kind: 'env', reason: 'serper_disabled' })
  })
