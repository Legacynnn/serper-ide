...
export function CommitMessageAiPane({
    ...
    sections.push(
      ...
      >
        <div className="space-y-0.5">
          <Label>Agent</Label>
          <p className="text-xs text-muted-foreground">
            Which agent drafts your commit messages. Serper invokes its CLI in the background, so
            the agent must be installed on the machine that hosts the worktree - your computer for
            local worktrees, or the SSH host for remote ones.
          </p>
        </div>
  ...
  if (
    ...
    matchesSettingsSearch(searchQuery, {
      ...
      description: 'Command line Serper runs to generate the commit message.',
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {activeCapability.modelSource === 'dynamic'
              ...
              : 'This agent does not expose model discovery, so Serper uses a manual catalog.'}
          </p>
  ...
  // Branch Prefix / Refresh Local Base Ref / Serper Attribution rows above.
  return (
