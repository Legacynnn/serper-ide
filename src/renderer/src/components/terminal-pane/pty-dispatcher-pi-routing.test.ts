...
describe('dispatcher → transport → onTitleChange for Pi spinner', () => {
  ...
  // internal redraw mid-turn. Serper's main process injects synthesized
  ...
  it('drops cursor-agent native "Cursor Agent" title so it cannot overwrite the synthesized spinner', async () => {
    ...
    // Simulate the realistic interleave: Serper injects a synthesized working
    // frame, cursor-agent re-emits its bare native title shortly after, Serper
  ...
  it('still surfaces the synthesized "Cursor ready" idle title after working', async () => {
    ...
    // decorated "Cursor ready" done frame Serper synthesizes on the `stop` hook.
    const { createIpcPtyTransport } = await import('./pty-transport')
    const onTitleChange = vi.fn()
