...
import { SERPER_EDITOR_REQUEST_CMD_SAVE_EVENT } from './editor-autosave'
...
export function useEditorCmdSaveRequest({
  ...
  useEffect(() => {
    ...
    window.addEventListener(SERPER_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
    return () => window.removeEventListener(SERPER_EDITOR_REQUEST_CMD_SAVE_EVENT, handler)
