...
export const SERPER_EDITOR_QUIESCE_FILE_SAVES_EVENT = 'serper:editor-quiesce-file-saves'
export const SERPER_EDITOR_EXTERNAL_FILE_CHANGE_EVENT = 'serper:editor-external-file-change'
export const SERPER_EDITOR_SAVE_FILE_EVENT = 'serper:editor-save-file'
export const SERPER_EDITOR_SAVE_AND_CLOSE_EVENT = 'serper:save-and-close'
export const SERPER_EDITOR_FILE_SAVED_EVENT = 'serper:editor-file-saved'
export const SERPER_EDITOR_REQUEST_CMD_SAVE_EVENT = 'serper:editor-request-cmd-save'
export const SERPER_EDITOR_REQUEST_FILE_CLOSE_EVENT = 'serper:editor-request-file-close'
...
export async function requestEditorSaveQuiesce(target: EditorSaveQuiesceTarget): Promise<void> {
  await new Promise<void>((resolve) => {
    ...
    window.dispatchEvent(
      new CustomEvent<EditorSaveQuiesceDetail>(SERPER_EDITOR_QUIESCE_FILE_SAVES_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export async function requestEditorFileSave(target: EditorSaveFileTarget): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    let claimed = false
    window.dispatchEvent(
      new CustomEvent<EditorSaveFileDetail>(SERPER_EDITOR_SAVE_FILE_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export function requestEditorFileClose(fileId: string): void {
  window.dispatchEvent(
    new CustomEvent<EditorRequestFileCloseDetail>(SERPER_EDITOR_REQUEST_FILE_CLOSE_EVENT, {
      detail: { fileId }
    })
  )
}
...
export function notifyEditorExternalFileChange(target: EditorPathMutationTarget): void {
  window.dispatchEvent(
    new CustomEvent<EditorPathMutationTarget>(SERPER_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, {
      detail: target
    })
  )
}
