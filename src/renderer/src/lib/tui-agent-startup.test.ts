...
describe('buildAgentDraftLaunchPlan', () => {
  ...
  it('uses SERPER_PI_PREFILL env var for pi (no CLI flag exists)', () => {
    // Why: pi has no `--prefill` flag, and bracketed-paste-after-ready races
    // against pi's lengthy startup output. The Serper overlay installs an
    // `serper-prefill` extension that reads SERPER_PI_PREFILL on session_start
    ...
    expect(
      ...
      agent: 'pi',
      launchCommand: 'pi; unset SERPER_PI_PREFILL',
      expectedProcess: 'pi',
      env: { SERPER_PI_PREFILL: 'https://github.com/acme/repo/issues/42' }
    })
  })
