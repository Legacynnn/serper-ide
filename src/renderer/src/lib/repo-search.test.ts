...
function makeRepo(overrides: Partial<Repo> = {}): Repo {
  return {
    id: 'repo-1',
    path: '/Users/test/src/serper',
    displayName: 'Legacynnn/serper',
    badgeColor: '#22c55e',
    addedAt: 0,
    ...overrides
  }
...
describe('repo-search', () => {
  ...
  it('matches display names case-insensitively', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'Legacynnn/serper', path: '/repos/serper' }),
      makeRepo({ id: '2', displayName: 'stablyai/noqa', path: '/repos/noqa' })
    ]

    expect(searchRepos(repos, 'SERPER').map((repo) => repo.id)).toEqual(['1'])
  })

  it('falls back to matching repo paths', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'frontend', path: '/src/team-a/serper' }),
  ...
  it('keeps display-name matches ahead of path-only matches', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'misc', path: '/src/serper-tools/misc' }),
      makeRepo({ id: '2', displayName: 'serper', path: '/src/team-a/project' })
    ]

    expect(searchRepos(repos, 'serper').map((repo) => repo.id)).toEqual(['2', '1'])
  })
})
