...
export function registerUpdaterBeforeUnloadBypass(): () => void {
  ...
  window.addEventListener(SERPER_UPDATER_QUIT_AND_INSTALL_STARTED_EVENT, markInProgress)
  window.addEventListener(SERPER_UPDATER_QUIT_AND_INSTALL_ABORTED_EVENT, clearInProgress)

  return () => {
    window.removeEventListener(SERPER_UPDATER_QUIT_AND_INSTALL_STARTED_EVENT, markInProgress)
    window.removeEventListener(SERPER_UPDATER_QUIT_AND_INSTALL_ABORTED_EVENT, clearInProgress)
