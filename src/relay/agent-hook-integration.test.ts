 ...
 *
 * Wires Serper's main-side SshChannelMultiplexer to the relay-side
...
describe('Integration: relay hook server → mux → AgentHookServer.ingestRemote', () => {
  ...
  let serperServer: AgentHookServer
  ...
  beforeEach(async () => {
    ...
    serperServer = new AgentHookServer()
    // Why: Serper-side never starts an HTTP server in this test — `ingestRemote`
  ...
  it('forwards a Claude UserPromptSubmit POST through to ingestRemote', async () => {
    ...
    serperServer.setListener((event) => {
      events.push({
