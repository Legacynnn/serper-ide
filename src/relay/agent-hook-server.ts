...
// a loopback HTTP server (same shape as Serper's main-process server: bind
...
const RELAY_HOOKS_DIR_NAME = '.serper-relay'
...
export type RelayHookServerOptions = {
  /** Where to put endpoint.env / endpoint.cmd. Defaults to `$HOME/.serper-relay/agent-hooks`. */
  endpointDir?: string
...
export class RelayAgentHookServer {
  ...
  // the original `source`/`env`/`version` so Serper's warn-once diagnostics fire
   ...
   *  forwards each entry as a fresh notification. Called after Serper has
  ...
  buildPtyEnv(): Record<string, string> {
    ...
    if (this.endpointFileWritten) {
      env.SERPER_AGENT_HOOK_ENDPOINT = this.endpointFilePath
    }
    return env
  }
