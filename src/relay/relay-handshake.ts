// Wire-level handshake helpers for the Serper relay.
//
...
export function setupDaemonHandshake(sock: Socket, cb: DaemonHandshakeCallbacks): void {
  ...
  ;(sock as Socket & { __serperOnHandshake?: typeof onHandshakeData }).__serperOnHandshake =
    onHandshakeData
}

export function detachHandshakeListener(sock: Socket): void {
  const tagged = sock as Socket & { __serperOnHandshake?: (chunk: Buffer) => void }
  if (tagged.__serperOnHandshake) {
    sock.removeListener('data', tagged.__serperOnHandshake)
    delete tagged.__serperOnHandshake
  }
}
...
function handleDaemonHandshakeFrame(
  ...
  if (msg.type !== 'serper-relay-handshake') {
    process.stderr.write(
  ...
  if (msg.version !== launchVersion) {
    ...
    try {
      sock.write(
        encodeHandshakeFrame({
          type: 'serper-relay-handshake-mismatch',
  ...
  sock.write(encodeHandshakeFrame({ type: 'serper-relay-handshake-ok', version: launchVersion }))
  return true
}
...
export function runConnectHandshake(
  ...
  const decoder: FrameDecoder = new FrameDecoder(
    (frame: DecodedFrame) => {
      ...
      if (msg.type === 'serper-relay-handshake-ok') {
      ...
      if (msg.type === 'serper-relay-handshake-mismatch') {
  ...
  sock.write(encodeHandshakeFrame({ type: 'serper-relay-handshake', version: myVersion }))
}
