...
describe('handshake framing', () => {
  it('round-trips an serper-relay-handshake envelope through the existing framing', () => {
    const sent = encodeHandshakeFrame({
      type: 'serper-relay-handshake',
    ...
    expect(msg).toEqual({ type: 'serper-relay-handshake', version: '0.1.0+deadbeef' })
  })

  it('round-trips an serper-relay-handshake-ok reply', () => {
    const sent = encodeHandshakeFrame({
      type: 'serper-relay-handshake-ok',
      version: '0.1.0+deadbeef'
    })
    const frames: DecodedFrame[] = []
    const decoder = new FrameDecoder((f) => frames.push(f))
    decoder.feed(sent)
    const msg = parseHandshakeMessage(frames[0].payload)
    expect(msg).toEqual({ type: 'serper-relay-handshake-ok', version: '0.1.0+deadbeef' })
  })

  it('round-trips an serper-relay-handshake-mismatch reply', () => {
    const sent = encodeHandshakeFrame({
  ...
  it('rejects payloads with unknown type', () => {
    const bogus = Buffer.from(JSON.stringify({ type: 'serper-something-else', version: 'x' }))
