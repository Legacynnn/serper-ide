#!/usr/bin/env node
/**
 * Terminal E2E helpers for agent-browser + CDP testing against a running Serper
 ...
 *
 * Usage:
 *   import { SerperTerminal } from './terminal-e2e-helpers.mjs'
 *
 *   const term = new SerperTerminal(9444)       // CDP port
 *   await term.connect()
...
export class SerperTerminal {
  /** @param {number} cdpPort — the --remote-debugging-port used when launching Serper */
  constructor(cdpPort) {
    this.port = cdpPort
  }
   ...
   */
  probePtyIdWithScreenshot(maxId = 10, screenshotPath = '/tmp/serper-pty-probe.png') {
    for (let i = 1; i <= maxId; i++) {
      evalInRenderer(
        this.port,
  ...
  /**
   * Take a screenshot of the Serper window.
   *
   * @param {string} path — output file path
   * @returns {string} the screenshot path
   */
  screenshot(path = '/tmp/serper-terminal.png') {
    ab(this.port, ['screenshot', path])
    return path
  }

  /**
   * Open a new terminal tab in Serper.
   * @returns {void}
   */
  newTerminal() {
  ...
  readLang(ptyId) {
    ...
    return this.screenshot('/tmp/serper-lang-check.png')
  }
}
...
if (process.argv[1]?.endsWith('terminal-e2e-helpers.mjs')) {
  ...
  const term = new SerperTerminal(port)
  console.log('Connecting to Serper on CDP port', port, '...')
  term.connect()
