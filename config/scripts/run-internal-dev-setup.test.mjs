...
describe('runInternalDevSetup', () => {
  it('does nothing when SERPER_INTERNAL_DEV_SETUP is unset', () => {
    const spawn = vi.fn()
  ...
  it('does nothing when the configured setup path does not exist', () => {
    ...
    expect(
      runInternalDevSetup({
        env: { SERPER_INTERNAL_DEV_SETUP: '/tmp/missing' },
        exists: () => false,
        spawn
      })
    ).toBe(0)
  ...
  it('skips non-executable setup paths on POSIX', () => {
    ...
    expect(
      runInternalDevSetup({
        env: { SERPER_INTERNAL_DEV_SETUP: '/tmp/setup' },
        platform: 'linux',
        exists: () => true,
        access: () => {
  ...
  it('runs the optional setup with the worktree path and ignores its exit status', () => {
    ...
    expect(
      runInternalDevSetup({
        env: {
          SERPER_INTERNAL_DEV_SETUP: '/tmp/setup',
          SERPER_WORKTREE_PATH: '/tmp/worktree'
        },
        platform: 'linux',
        exists: () => true,
        access: () => undefined,
  ...
  it('uses cmd.exe with explicit quoting so .cmd setup shims receive paths with spaces', () => {
    ...
    expect(
      runInternalDevSetup({
        env: {
          SERPER_INTERNAL_DEV_SETUP: 'C:\\tools\\setup.cmd',
          SERPER_WORKTREE_PATH: 'C:\\repo\\worktree'
        },
        platform: 'win32',
        exists: () => true,
        spawn
