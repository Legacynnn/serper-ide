...
const useStableElectronName =
  process.env.SERPER_DEV_STABLE_NAME === '1' || rawForwardedArgs.includes(STABLE_NAME_FLAG)
const forwardedRaw = rawForwardedArgs.filter((arg) => arg !== STABLE_NAME_FLAG)
if (useStableElectronName) {
  process.env.SERPER_DEV_STABLE_NAME = '1'
}
...
function createDockTitle(branch, label) {
  return `Serper: ${branch || label || 'dev'}`
}

function seedDevInstanceIdentityEnv() {
  const branch =
    process.env.SERPER_DEV_BRANCH ||
    readGitValue(['symbolic-ref', '--quiet', '--short', 'HEAD']) ||
    readGitValue(['rev-parse', '--short', 'HEAD'])
  const worktreeName = process.env.SERPER_DEV_WORKTREE_NAME || path.basename(repoRoot)
  const label = process.env.SERPER_DEV_INSTANCE_LABEL || formatDevInstanceLabel(branch, worktreeName)
  const identitySeed = process.env.SERPER_DEV_INSTANCE_KEY || repoRoot
  const dockTitle = process.env.SERPER_DEV_DOCK_TITLE || createDockTitle(branch, label)

  process.env.SERPER_DEV_REPO_ROOT ||= repoRoot
  process.env.SERPER_DEV_INSTANCE_KEY ||= identitySeed
  if (branch) {
    process.env.SERPER_DEV_BRANCH ||= branch
  }
  if (worktreeName) {
    process.env.SERPER_DEV_WORKTREE_NAME ||= worktreeName
  }
  if (label) {
    ...
    process.env.SERPER_DEV_INSTANCE_LABEL ||= label
  }
  process.env.SERPER_DEV_DOCK_TITLE ||= dockTitle
}
...
function prepareMacDevElectronApp() {
  ...
  const title = process.env.SERPER_DEV_DOCK_TITLE || 'Serper: dev'
  const identityKey = process.env.SERPER_DEV_INSTANCE_KEY || repoRoot
  ...
  const markerPath = path.join(distDir, 'serper-dev-electron-app.json')
  const bundleId = `com.legacynnn.serper.dev.${sanitizeBundleIdPart(hash)}`
...
function getDevUserDataPath() {
  if (process.env.SERPER_DEV_USER_DATA_PATH) {
    return process.env.SERPER_DEV_USER_DATA_PATH
  }
  if (process.platform === 'darwin') {
    return path.join(process.env.HOME ?? '', 'Library', 'Application Support', 'serper-dev')
  }
...
function prepareDevCliWrapper() {
  ...
  if (process.platform === 'win32') {
    writeFileSync(
      path.join(binDir, 'serper-dev.cmd'),
      `@echo off\r\nset "SERPER_USER_DATA_PATH=${userDataPath}"\r\nset "SERPER_APP_EXECUTABLE=${electronBin}"\r\nset "SERPER_APP_EXECUTABLE_NEEDS_APP_ROOT=1"\r\nnode "${cliPath}" %*\r\n`,
      'utf8'
    )
  } else {
    const wrapperPath = path.join(binDir, 'serper-dev')
    writeFileSync(
      wrapperPath,
      `#!/usr/bin/env bash\nexport SERPER_USER_DATA_PATH=${JSON.stringify(userDataPath)}\nexport SERPER_APP_EXECUTABLE=${JSON.stringify(electronBin)}\nexport SERPER_APP_EXECUTABLE_NEEDS_APP_ROOT=1\nexec node ${JSON.stringify(cliPath)} "$@"\n`,
      'utf8'
    )
  ...
  console.log(`[serper-dev] Prepared wrapper in ${binDir}`)
}
...
if (process.env.SERPER_SKIP_DEV_CLI_PREPARE !== '1') {
  prepareDevCliWrapper()
}

seedDevInstanceIdentityEnv()
if (!useStableElectronName && process.env.SERPER_SKIP_DEV_ELECTRON_APP_PREPARE !== '1') {
...
function prepareDevWebClient() {
  if (process.env.SERPER_SKIP_DEV_WEB_PREPARE === '1' || isHelpOrVersion) {
    return
  }
  ...
  if (!existsSync(getDevWebClientIndexPath()) && process.env.SERPER_DEV_WEB_PREPARE !== '1') {
    console.error(
      '[serper-dev] Web client bundle missing; skipping pairing web build. Run `pnpm run build:web` or set SERPER_DEV_WEB_PREPARE=1 when you need browser pairing.'
    )
    return
  }
  ...
  console.error('[serper-dev] Building web client for pairing...')
  execFileSync(
    process.execPath,
...
if (!isHelpOrVersion && process.env.SERPER_DEV_INSTANCE_LABEL) {
  console.error(`[serper-dev] Instance: ${process.env.SERPER_DEV_INSTANCE_LABEL}`)
}
let forwardedExtras = []
if (!userPassedPort && !isHelpOrVersion) {
  ...
  if (envPortRaw) {
    ...
    if (port === null) {
      console.error(
        `[serper-dev] Ignoring invalid REMOTE_DEBUGGING_PORT=${JSON.stringify(envPortRaw)}; falling back to probe.`
      )
    }
  }
  if (port === null) {
  ...
  if (port !== null) {
    ...
    console.error(`[serper-dev] Remote debugging on http://127.0.0.1:${port}`)
  } else {
    console.error(
      '[serper-dev] No free debug port found in sweep; starting without --remote-debugging-port.'
    )
  }
}
prepareDevWebClient()
