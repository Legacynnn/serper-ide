...
# no saved sessions). Your real `serper-dev` profile is left untouched.
#
# Usage:
...
#   SERPER_FRESH_PROFILE_DIR=/some/path ./config/scripts/dev-fresh-profile.sh   # use a fixed dir
set -euo pipefail
...
PROFILE_DIR="${SERPER_FRESH_PROFILE_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/serper-fresh-profile.XXXXXXXX")}"
mkdir -p "$PROFILE_DIR"

cleanup() {
  if [[ "$KEEP" -eq 0 && -z "${SERPER_FRESH_PROFILE_DIR:-}" ]]; then
    # Guard rm -rf against accidental empty/unrelated PROFILE_DIR.
    [[ -n "${PROFILE_DIR:-}" && -d "$PROFILE_DIR" && "$PROFILE_DIR" == */serper-fresh-profile* ]] || return 0
    rm -rf "$PROFILE_DIR"
...
SERPER_DEV_USER_DATA_PATH="$PROFILE_DIR" pnpm dev
