...
const binaryPath = path.join(packagePath, '.build', 'release', 'serper-computer-use-macos')
const appPath = path.join(packagePath, '.build', 'release', 'Serper Computer Use.app')
const appExecutablePath = path.join(appPath, 'Contents', 'MacOS', 'serper-computer-use-macos')
...
const bundleId = process.env.SERPER_COMPUTER_MACOS_BUNDLE_ID ?? 'com.legacynnn.serper.computer-use'
const displayName = 'Serper Computer Use'
...
function buildUniversalBinary() {
  const builtBinaries = universalTriples.map((triple) => {
    ...
    return path.join(packagePath, '.build', triple, 'release', 'serper-computer-use-macos')
  })
...
function codesignArgs(identity, targetPath) {
  ...
  if (process.env.SERPER_MAC_RELEASE === '1' && identity !== '-') {
...
function resolveSigningIdentity() {
  const explicitIdentity = process.env.SERPER_COMPUTER_MACOS_SIGN_IDENTITY ?? process.env.CSC_NAME
  if (explicitIdentity) {
  ...
  if (process.env.SERPER_MAC_RELEASE !== '1' && developmentMatch) {
...
<dict>
  ...
  <string>serper-computer-use-macos</string>
  ...
  <string>Serper Computer Use needs Accessibility permission to read and interact with app interfaces when you ask Serper to use apps.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>Serper Computer Use needs Screen Recording permission to capture app windows when you ask Serper to inspect your screen.</string>
</dict>
</plist>
`
}
